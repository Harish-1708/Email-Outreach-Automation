...
def _connect_tracker_worksheet(spreadsheet_id: str, worksheet_name: str):
    ...
    updates one column's cells, never touches its header or adds
    columns to it."""
    client, gspread_module = _build_gspread_client()
    spreadsheet = _open_spreadsheet_with_retries(client, spreadsheet_id)
    try:
        raw_ws = spreadsheet.worksheet(worksheet_name)
    except gspread_module.exceptions.WorksheetNotFound:
        raise RuntimeError(
            f"Creator Tracker worksheet '{worksheet_name}' not found in spreadsheet {spreadsheet_id}. "
            "Check the CREATOR_TRACKER_WORKSHEET_NAME secret, and that the sheet has been shared "
