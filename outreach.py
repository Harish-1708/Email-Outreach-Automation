...
def canonical_account_name(name: str, accounts: Dict[str, Dict[str, str]]) -> Optional[str]:
    ...
    EMAIL_ACCOUNTS_JSON, matching case-insensitively, or None if there
    is no such account at all.

    Account names are typed by hand in three separate places that have
    no way to validate each other: the EMAIL_ACCOUNTS_JSON secret, a
    campaign's sending.rotation_accounts config, and the per-lead
    SenderAccount column in the Master Sheet. A real, confirmed
    production config has slots named "Sales2"/"Sales3"/"Sales4" while
    lead rows carry "sales1"-style lowercase values — so a plain
    case-sensitive `in accounts` check treats the same account as two
    different ones. Everything downstream keys off the canonical name
    returned here, so a case variant can never fork into a second
    identity."""
    if not name:
        return None
    if name in accounts:
