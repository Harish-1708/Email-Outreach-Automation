...
def send_batch(campaign_cfg: Dict, sheets: SheetsConnector, accounts: Dict[str, Dict[str, str]],
    ...
    (sent, error, or skipped) is logged to SendLog under one BatchID, and
    every error is also classified and logged to the Error Log.

    Sender account selection (per lead) uses resolve_sender_account_for_send:
    manual SenderAccount always wins; otherwise sending.sender_rotation
    auto-picks the least-used eligible account; otherwise the single
    campaign/global default. A per-account daily cap (sending.
    per_account_daily_limit) applies uniformly regardless of how the
    account was chosen — a lead whose only eligible account(s) are already
    at capacity is marked "skipped" (not "error") and simply retried on a
    later run.
    """
    stages = campaign_cfg["stages"]
    ...
    for i, item in enumerate(plan):
        ...
        try:
            ...
            account_name = resolve_sender_account_for_send(lead, campaign_cfg, accounts,
                                                             sent_today_by_account, batch_assigned_counts)
            ...
            results.append({"lead_id": lead_id, "email": lead_email, "status": outcome_status,
            ...
            try:
                ...
                batch_assigned_counts[account_name] = batch_assigned_counts.get(account_name, 0) + 1
                ...
                # The send genuinely happened, so this account WAS used —
                # count it even though the sheet write failed, so rotation/
                # capacity tracking for the rest of THIS batch stays accurate.
                batch_assigned_counts[account_name] = batch_assigned_counts.get(account_name, 0) + 1
