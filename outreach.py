...
def sync_campaign_to_tracker_sheet(sheets: SheetsConnector, campaign_cfg: Dict, tracker_ws) -> Dict:
    ...
    touches any column other than Contact Status and Last Contacted
    Date — this sheet's rows and every other column are owned and
    populated by a different daily process. A lead with no matching
    row, or an ambiguous name match, is reported as a warning for
    review — never silently skipped, and never guessed at.
