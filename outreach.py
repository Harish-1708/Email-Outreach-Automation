...
def build_tracker_row_index(tracker_rows: List[Dict]) -> Tuple[Dict[str, int], Dict[str, List[int]]]:
    """Builds {email_lower: row_number} and {name_lower: [row_numbers]}
    lookups from the tracker sheet's own current rows (each dict must
    include '_row'). Name maps to a LIST, not a single row, because
    names can collide across a sheet spanning thousands of rows and
    many campaigns — see find_tracker_row_for_lead, which treats more
    than one name match as ambiguous rather than guessing which row is
    the right one."""
...
def find_tracker_row_for_lead(lead: Dict, email_index: Dict[str, int],
                               name_index: Dict[str, List[int]]) -> Dict:
    """Tries EMAIL first (unique, reliable when present), falls back to
    NAME — matching this tracker sheet's own mixed reliability, where
    either column can be blank for a given row. Tries the lead's own
    ...
    A name match against MORE than one tracker row is reported as
    ambiguous and never guessed at — silently picking one could update
    the wrong creator's status in a sheet spanning thousands of
    unrelated rows across every other campaign too.

    Returns {"row": int or None, "matched_by": "email"/"name"/None,
    "ambiguous": bool}."""
    email = _safe_lead_str(lead.get("Email")).lower()
    if email and email in email_index:
        return {"row": email_index[email], "matched_by": "email", "ambiguous": False}
...
def sync_campaign_to_tracker_sheet(sheets: SheetsConnector, campaign_cfg: Dict, tracker_ws) -> Dict:
    ...
    for lead in sheets.get_all_leads():
        ...
        match = find_tracker_row_for_lead(lead, email_index, name_index)
