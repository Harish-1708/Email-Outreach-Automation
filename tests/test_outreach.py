...
# A dedicated, permanent fixture — NEVER the live Kelson_Creators_Licensing
# directory — for any test that needs a guaranteed-complete set of all 20
# stage x variant files. The live campaign is real, user-editable data:
# Delete Stage / Delete Variant can (and did, in production) legitimately
# reduce its actual file count below 20, which would otherwise break these
# tests permanently on every future push, for a reason that has nothing to
# do with a real regression.
FULL_TEMPLATE_SET_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "full_template_set")
...
def test_approval_no_longer_gates_eligibility():
    """Approval used to be a required gate; it's now informational
    only — a lead with Approval=No, Pending, or blank must still be
    eligible as long as everything else checks out."""
    for approval_value in ("Pending", "", "No", "Yes"):
        eligible = outreach.get_eligible_leads([make_lead(Approval=approval_value)], STAGES, 0)
        assert len(eligible) == 1, f"Approval={approval_value!r} should not block eligibility"
...
def test_duplicate_email_rows_dedup_only_applies_among_leads_that_pass_other_checks():
    # Row1 fails on Status (terminal) and is filtered out BEFORE the dedup
    # step — it never "claims" the email slot, so row2 (which does pass)
    # is correctly still eligible. Dedup only applies among leads that
    # already passed every other check, not globally by row order.
    row1 = make_lead(_row=2, LeadID="L1", Email="same@abc.com", Status="Stopped - Bounced")
    row2 = make_lead(_row=3, LeadID="L2", Email="same@abc.com")
    eligible = outreach.get_eligible_leads([row1, row2], STAGES, 0)
    assert len(eligible) == 1
...
def test_ignore_wait_days_still_respects_every_other_eligibility_rule():
    ...
    no_email = make_lead(IntroSentAt=recent, Email="")

    eligible = outreach.get_eligible_leads(
        [already_sent_this_stage, replied, stopped, no_email], STAGES, 1, ignore_wait_days=True)
    assert eligible == []
...
def test_import_leads_skips_duplicate_email_by_default():
    existing = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([existing])
    new_leads = [{"Email": "rocky@abc.com", "Video File": "https://tiktok.com/video2"}]

    summary = outreach.import_leads(fake_sheets, "TestCampaign", new_leads)

    assert summary == {"imported": 0, "skipped_duplicate": 1, "skipped_no_email": 0}
    assert len(fake_sheets.get_all_leads()) == 1  # nothing new added


def test_import_leads_allow_duplicate_emails_adds_a_second_row_for_the_same_creator():
    """The actual feature request: contacting the same creator again
    for a genuinely different video needs its own row (and eventually
    its own Asana task), without disturbing the first row at all."""
    existing = make_lead(_row=2, LeadID="1", Email="rocky@abc.com", **{"Video File": "video1"})
    fake_sheets = FakeSheets([existing])
    new_leads = [{"Email": "rocky@abc.com", "Video File": "video2"}]

    summary = outreach.import_leads(fake_sheets, "TestCampaign", new_leads, allow_duplicate_emails=True)

    assert summary == {"imported": 1, "skipped_duplicate": 0, "skipped_no_email": 0}
    all_leads = fake_sheets.get_all_leads()
    assert len(all_leads) == 2
    assert {l.get("Video File") for l in all_leads} == {"video1", "video2"}
    # The original row is completely untouched.
    original = next(l for l in all_leads if l["LeadID"] == "1")
    assert original["Video File"] == "video1"


def test_import_leads_allow_duplicate_emails_still_skips_blank_email():
    fake_sheets = FakeSheets([])
    new_leads = [{"Email": "", "Video File": "video1"}]
...
def test_import_leads_allow_duplicate_emails_true_does_not_affect_send_eligibility():
    """The actual safety guarantee this whole feature leans on: sending
    is governed entirely by find_duplicate_email_leads / get_eligible_
    leads, completely independent of whether import allowed the
    duplicate in — this must never change, regardless of the import
    flag used to get two rows into the sheet."""
    first = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    second = make_lead(_row=3, LeadID="2", Email="rocky@abc.com")
    eligible = outreach.get_eligible_leads([first, second], STAGES, 0)
    assert len(eligible) == 1
    assert eligible[0]["LeadID"] == "1"  # only the first row is ever eligible to send


def test_cmd_import_leads_reads_allow_duplicate_emails_from_payload(tmp_path, monkeypatch):
    existing = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([existing])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    payload_file = tmp_path / "payload.json"
    payload_file.write_text(json.dumps({
        "leads": [{"Email": "rocky@abc.com", "Video File": "video2"}],
        "allow_duplicate_emails": True,
    }))

    args = argparse.Namespace(campaign="Foo", file=str(payload_file))
    outreach.cmd_import_leads(args)

    assert len(fake_sheets.get_all_leads()) == 2


def test_cmd_import_leads_defaults_to_false_when_key_absent_from_payload(tmp_path, monkeypatch):
    existing = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([existing])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    payload_file = tmp_path / "payload.json"
    payload_file.write_text(json.dumps({"leads": [{"Email": "rocky@abc.com"}]}))  # no allow_duplicate_emails key

    args = argparse.Namespace(campaign="Foo", file=str(payload_file))
    outreach.cmd_import_leads(args)

    assert len(fake_sheets.get_all_leads()) == 1  # still skipped — old behavior preserved



    """The actual reported bug: a custom field name never seen before on
    this campaign's Sheet was silently dropped, since append_lead can
    only fill in columns the header already has. import_leads must
    widen the header FIRST, for every field name across every incoming
    lead, before appending any rows."""
    fake_sheets = FakeSheets([])
    new_leads = [{"Email": "a@abc.com", "Client": "DudeRobe", "Product": "SheRobe"}]

    outreach.import_leads(fake_sheets, "TestCampaign", new_leads)

    assert len(fake_sheets.header_widen_calls) == 1
    assert set(fake_sheets.header_widen_calls[0]) == {"Email", "Client", "Product"}


def test_import_leads_header_widening_happens_before_any_row_is_appended():
    """Order matters — widening after appending would mean the FIRST
    row's custom fields still get dropped, since append_lead reads
    whatever the header is AT THE MOMENT it runs."""
    call_order = []
    fake_sheets = FakeSheets([])
    original_ensure = fake_sheets.ensure_master_header_includes
    original_append = fake_sheets.append_lead

    def tracked_ensure(column_names):
        call_order.append("widen")
        original_ensure(column_names)

    def tracked_append(fields):
        call_order.append("append")
        original_append(fields)

    fake_sheets.ensure_master_header_includes = tracked_ensure
    fake_sheets.append_lead = tracked_append

    outreach.import_leads(fake_sheets, "TestCampaign", [{"Email": "a@abc.com", "Client": "DudeRobe"}])

    assert call_order == ["widen", "append"]


def test_import_leads_widens_header_across_all_leads_not_just_the_first():
    """Different rows can map different custom columns — the widening
    must cover the UNION of every field name across every lead in the
    batch, not just whatever the first row happened to have."""
    fake_sheets = FakeSheets([])
    new_leads = [
        {"Email": "a@abc.com", "Client": "DudeRobe"},
        {"Email": "b@abc.com", "Product": "SheRobe"},
    ]

    outreach.import_leads(fake_sheets, "TestCampaign", new_leads)

    all_widened = set(fake_sheets.header_widen_calls[0])
    assert "Client" in all_widened
    assert "Product" in all_widened


def test_ensure_master_header_includes_only_adds_missing_columns(monkeypatch):
    class FakeMasterWs:
        def __init__(self):
            self.header = ["Email", "FirstName"]
            self.updated_cells = []
            self.col_count = 20
            self.resize_calls = []
    ...
    assert sheets.master_ws.updated_cells == [(1, 3, "Client"), (1, 4, "Product")]  # Email already there, skipped
    assert sheets.master_ws.header == ["Email", "FirstName", "Client", "Product"]
    assert sheets.master_ws.resize_calls == []  # plenty of room already (col_count=20) — no resize needed


def test_ensure_master_header_includes_expands_grid_when_it_would_otherwise_overflow():
    """The actual production bug: a sheet whose grid was allocated a
    fixed column count long ago (38, in the real incident) rejects a
    write beyond that with a real 400 error from Google, regardless of
    how correct the header content is. The grid itself must be resized
    FIRST, whenever the new columns would exceed the CURRENT count —
    not just the header row's own content."""
    class FakeMasterWs:
        def __init__(self):
            self.header = [f"Col{i}" for i in range(38)]  # exactly at the real incident's limit
            self.updated_cells = []
            self.col_count = 38
            self.resize_calls = []
        ...
        def update_cell(self, row, col, value):
            if col > self.col_count:
                raise RuntimeError(f"exceeds grid limits: max columns {self.col_count}")
            self.updated_cells.append((row, col, value))
            while len(self.header) < col:
                self.header.append("")
            self.header[col - 1] = value
    ...
    sheets.ensure_master_header_includes(["Client", "Product"])  # 2 new columns, would need 40

    assert sheets.master_ws.resize_calls == [50]  # 40 needed + 10 headroom
    assert sheets.master_ws.updated_cells == [(1, 39, "Client"), (1, 40, "Product")]


def test_ensure_master_header_includes_no_op_when_nothing_missing():
    class FakeMasterWs:
        def __init__(self):
            self.header = ["Email", "Client"]
            self.updated_cells = []
            self.col_count = 20
            self.resize_calls = []
    ...
    assert sheets.master_ws.updated_cells == []
    assert sheets.master_ws.resize_calls == []
...
# ---------- cmd_set_lead_override ----------

def test_set_lead_override_sets_stopped_manual_status(monkeypatch, capsys):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com",
                               status=outreach.STATUS_STOPPED_MANUAL, asana_stage=None,
                               reply_status=None, last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["Status"] == "Stopped - Manual"


def test_set_lead_override_stopped_manual_actually_excludes_from_sending():
    """The real point of this status — must actually behave as a
    terminal status, excluding the lead from get_eligible_leads,
    exactly like the other Stopped statuses."""
    lead = make_lead(Email="rocky@abc.com", Status=outreach.STATUS_STOPPED_MANUAL)
    eligible = outreach.get_eligible_leads([lead], STAGES, 0)
    assert eligible == []


def test_set_lead_override_empty_status_clears_it(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com", Status="Stopped - Manual")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status="", asana_stage=None, reply_status=None, last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)
...
def test_set_lead_override_rejects_an_arbitrary_status_value(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status="Whatever I Want",
                               asana_stage=None, reply_status=None, last_inbound_classification=None)
    with pytest.raises(SystemExit):
        outreach.cmd_set_lead_override(args)

    unchanged = fake_sheets.get_all_leads()[0]
    assert unchanged.get("Status", "") != "Whatever I Want"


def test_set_lead_override_sets_asana_stage(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None,
                               asana_stage="Rights Secured", reply_status=None, last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["ManualAsanaStage"] == "Rights Secured"


def test_set_lead_override_rejects_an_unrecognized_asana_stage(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None,
                               asana_stage="Finalized!!", reply_status=None, last_inbound_classification=None)
    with pytest.raises(SystemExit):
        outreach.cmd_set_lead_override(args)


def test_set_lead_override_no_matching_email_exits_nonzero(monkeypatch):
    fake_sheets = FakeSheets([])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="nobody@abc.com", status="Stopped - Manual",
                               asana_stage=None, reply_status=None, last_inbound_classification=None)
    with pytest.raises(SystemExit):
        outreach.cmd_set_lead_override(args)


def test_set_lead_override_can_set_both_fields_at_once(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com",
                               status=outreach.STATUS_STOPPED_MANUAL, asana_stage="Rights Secured",
                               reply_status=None, last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)
...
def test_set_lead_override_sets_reply_status_and_reply_at(monkeypatch):
    """The actual reported gap: a genuine reply on a NEW thread the
    automated checker can't match back to this lead — setting
    ReplyStatus directly must also set ReplyAt, matching what the
    normal automatic detection flow does."""
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None, asana_stage=None,
                               reply_status="Replied", last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["ReplyStatus"] == "Replied"
    assert updated["ReplyAt"]  # a real timestamp got written, not left blank


def test_set_lead_override_reply_status_flows_through_to_negotiating():
    """The end-to-end point of the whole feature — once ReplyStatus is
    set this way, compute_lead_asana_stage treats it exactly like a
    normally-detected reply and reaches Negotiating, which never
    reverts afterward regardless of send data."""
    lead = make_lead(Email="rocky@abc.com", ReplyStatus="Replied", IntroSentAt="2026-08-01 09:00:00")
    assert outreach.compute_lead_asana_stage(lead) == outreach.ASANA_STAGE_NEGOTIATING


def test_set_lead_override_clearing_reply_status_does_not_touch_reply_at(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com", ReplyAt="2026-09-01 10:00:00")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)

    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None, asana_stage=None,
                               reply_status="", last_inbound_classification=None)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["ReplyStatus"] == ""
    assert updated["ReplyAt"] == "2026-09-01 10:00:00"  # historical timestamp preserved


def test_set_lead_override_rejects_an_invalid_reply_status(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None, asana_stage=None,
                               reply_status="Maybe", last_inbound_classification=None)
    with pytest.raises(SystemExit):
        outreach.cmd_set_lead_override(args)

    unchanged = fake_sheets.get_all_leads()[0]
    assert unchanged.get("ReplyStatus", "") != "Maybe"


def test_set_lead_override_sets_last_inbound_classification_and_at(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None, asana_stage=None,
                               reply_status=None, last_inbound_classification=outreach.CLASSIFICATION_GENUINE)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["LastInboundClassification"] == "Genuine Reply"
    assert updated["LastInboundAt"]


def test_set_lead_override_rejects_an_invalid_classification(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com", status=None, asana_stage=None,
                               reply_status=None, last_inbound_classification="Something Else")
    with pytest.raises(SystemExit):
        outreach.cmd_set_lead_override(args)


def test_set_lead_override_can_set_all_four_fields_at_once(monkeypatch):
    lead = make_lead(_row=2, LeadID="1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    ...
    args = argparse.Namespace(campaign="Foo", email="rocky@abc.com",
                               status=outreach.STATUS_STOPPED_MANUAL, asana_stage="Negotiating",
                               reply_status="Replied", last_inbound_classification=outreach.CLASSIFICATION_GENUINE)
    outreach.cmd_set_lead_override(args)

    updated = fake_sheets.get_all_leads()[0]
    assert updated["Status"] == "Stopped - Manual"
    assert updated["ManualAsanaStage"] == "Negotiating"
    assert updated["ReplyStatus"] == "Replied"
    assert updated["LastInboundClassification"] == "Genuine Reply"
...
def test_all_20_templates_load_and_render_without_leftover_placeholders_even_with_blank_fields():
    ...
    # template have its own subject". Uses the dedicated fixture (see
    # FULL_TEMPLATE_SET_DIR) rather than the live campaign, since the
    # live one's actual file count can legitimately change via Delete
    # Stage / Delete Variant.
    ...
    for stage in stages:
        for variant in variants:
            rendered = outreach.render_email(FULL_TEMPLATE_SET_DIR, stage, variant, lead,
...
def test_templates_contain_no_diaz_or_event_branding():
    ...
    for stage in stages:
        for variant in variants:
            path = os.path.join(FULL_TEMPLATE_SET_DIR, f"{stage}_{variant}.txt")
...
def test_decide_asana_sync_action_explicit_override_can_move_out_of_rights_secured():
    """The actual reported production bug: a lead correctly set to
    ManualAsanaStage=Negotiating from the Data tab, whose task
    currently sits in Rights Secured in Asana, must actually be able
    to move — otherwise the override feature can never do the one
    thing it exists for, and the lead stays silently stuck forever
    while sync keeps reporting 'updated' with nothing changing."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ManualAsanaStage="Negotiating")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Rights Secured")
    assert decision["action"] == "update"
    assert decision["target_stage"] == "Negotiating"


def test_decide_asana_sync_action_explicit_override_can_move_out_of_declined_dead():
    lead = _lead_for_asana(AsanaTaskGID="123456", ManualAsanaStage="Follow-up")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Declined / Dead")
    assert decision["action"] == "update"
    assert decision["target_stage"] == "Follow-up"


# ---------- automatic stage REGRESSION (same lead in two campaigns) ----------

def test_decide_asana_sync_action_never_regresses_task_backward_automatically():
    """The actual reported root cause, finally isolated: the same
    creator email existing in BOTH an older completed campaign and the
    current active one, with the SAME AsanaTaskGID on both rows — so
    both campaigns' syncs drive the same Asana task and whichever runs
    last wins. The active campaign advances it to Negotiating on a
    reply; the old campaign, which has no record of that reply and so
    computes only "Outreach Sent" from its own send data, drags it
    straight back. That loop repeats forever, which is why the task
    kept reverting no matter how many times it was fixed by hand."""
    # This lead's own data only justifies Outreach Sent (no reply on
    # THIS campaign's sheet) — but the task is already at Negotiating.
    lead = _lead_for_asana(AsanaTaskGID="123456", IntroSentAt="2026-09-01 09:00:00")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Negotiating")
    assert decision["action"] == "update"
    assert decision["target_stage"] is None  # never dragged back to Outreach Sent


def test_decide_asana_sync_action_forward_progress_still_works_normally():
    """The no-regression rule must never block legitimate FORWARD
    movement — that's the entire normal operation of the sync."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ReplyStatus="Replied",
                            IntroSentAt="2026-09-01 09:00:00")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Outreach Sent")
    assert decision["target_stage"] == outreach.ASANA_STAGE_NEGOTIATING


def test_decide_asana_sync_action_explicit_override_can_still_move_backward():
    """A deliberate human override must still be able to move a task
    backward — only AUTOMATIC regression is blocked."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ManualAsanaStage="Outreach Sent")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Negotiating")
    assert decision["target_stage"] == "Outreach Sent"


def test_decide_asana_sync_action_same_stage_is_not_treated_as_regression():
    lead = _lead_for_asana(AsanaTaskGID="123456", IntroSentAt="2026-09-01 09:00:00")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Outreach Sent")
    assert decision["target_stage"] == "Outreach Sent"


def test_asana_stage_rank_returns_none_for_stages_outside_progression():
    """Terminal manual-only stages and unrecognized custom sections must
    return None so the regression check correctly skips them rather
    than comparing against a meaningless position."""
    assert outreach._asana_stage_rank("Rights Secured") is None
    assert outreach._asana_stage_rank("Declined / Dead") is None
    assert outreach._asana_stage_rank("Some Custom Section A Human Added") is None
    assert outreach._asana_stage_rank("Sourced") == 0
    assert outreach._asana_stage_rank("Negotiating") == 3


def test_decide_asana_sync_action_still_protects_when_override_blank():
    """The original guarantee must still hold for the case it was built
    for — an AUTOMATIC recomputation (no explicit override at all)
    must never move a task out of a manual-only stage on its own."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ReplyStatus="Replied", ManualAsanaStage="")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Rights Secured")
    assert decision["target_stage"] is None


def test_decide_asana_sync_action_unrecognized_override_does_not_bypass_protection():
    """A typo'd override value falls through to auto-derivation inside
    compute_lead_asana_stage — it must NOT also count as an 'explicit
    override' for bypassing the manual-only-stage protection, or a
    typo could accidentally move a task that was never meant to move."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ManualAsanaStage="Finalized!!")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Rights Secured")
    assert decision["target_stage"] is None


def test_decide_asana_sync_action_override_to_the_same_manual_stage_is_a_no_op_move():
    """Setting the override to the stage the task is ALREADY sitting in
    (e.g. re-confirming Rights Secured) should compute target_stage
    equal to the current section — the caller's own "only move if
    different" check (in sync_campaign_to_asana) is what keeps this
    from generating a pointless API call, not this function."""
    lead = _lead_for_asana(AsanaTaskGID="123456", ManualAsanaStage="Rights Secured")
    decision = outreach.decide_asana_sync_action(lead, current_asana_section_name="Rights Secured")
    assert decision["target_stage"] == "Rights Secured"
...
def test_parse_date_to_iso_handles_the_exact_reported_format():
    """The actual reported bug: '09/03/26' (US-style MM/DD/YY, a common
    Sheet display format) was never being converted — the old approach
    of taking the first 10 characters only worked when the value
    already happened to be ISO-formatted."""
    assert outreach._parse_date_to_iso("09/03/26") == "2026-09-03"


def test_parse_date_to_iso_handles_four_digit_year():
    assert outreach._parse_date_to_iso("09/03/2026") == "2026-09-03"


def test_parse_date_to_iso_passes_through_already_iso_format():
    assert outreach._parse_date_to_iso("2026-09-03") == "2026-09-03"


def test_parse_date_to_iso_handles_month_name_format():
    assert outreach._parse_date_to_iso("September 3, 2026") == "2026-09-03"


def test_parse_date_to_iso_returns_none_for_unparseable_value():
    assert outreach._parse_date_to_iso("not a date") is None


def test_build_custom_fields_payload_converts_non_iso_date_before_sending():
    """The end-to-end fix — a Last Contact Date typed exactly like the
    real reported case must reach Asana as valid ISO, not the raw
    Sheet-display string."""
    lead = _lead_for_asana(**{"Last Contact Date": "09/03/26"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-03"}


def test_build_custom_fields_payload_skips_date_field_when_unparseable():
    lead = _lead_for_asana(**{"Last Contact Date": "garbage"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert "f_date" not in payload


def test_build_custom_fields_payload_last_contact_date_uses_last_action_at_when_more_recent():
    """The actual reported bug: sending a real follow-up today must
    move this field forward, even though the Sheet's own manually-
    typed 'Last Contact Date' column is stale — the whole point is
    that a real send should never leave this field frozen."""
    lead = _lead_for_asana(**{"Last Contact Date": "2026-09-03", "LastActionAt": "2026-09-05 10:30:00"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-05"}


def test_build_custom_fields_payload_last_contact_date_keeps_sheet_value_when_more_recent():
    """The reverse direction — a manually-typed, more recent date must
    never be overwritten by an older LastActionAt."""
    lead = _lead_for_asana(**{"Last Contact Date": "2026-09-05", "LastActionAt": "2026-09-01 10:30:00"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-05"}


def test_build_custom_fields_payload_last_contact_date_from_last_action_at_when_sheet_blank():
    lead = _lead_for_asana(**{"Last Contact Date": "", "LastActionAt": "2026-09-05 10:30:00"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-05"}


def test_build_custom_fields_payload_last_contact_date_from_sheet_when_last_action_at_blank():
    lead = _lead_for_asana(**{"Last Contact Date": "2026-09-03", "LastActionAt": ""})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-03"}


def test_build_custom_fields_payload_last_contact_date_blank_when_neither_source_available():
    lead = _lead_for_asana(**{"Last Contact Date": "", "LastActionAt": ""})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert "f_date" not in payload


def test_build_custom_fields_payload_last_contact_date_ignores_unparseable_sheet_value_but_still_uses_last_action_at():
    lead = _lead_for_asana(**{"Last Contact Date": "garbage", "LastActionAt": "2026-09-05 10:30:00"})
    payload = outreach.build_asana_custom_fields_payload(lead, _field_defs())
    assert payload["f_date"] == {"date": "2026-09-05"}
...
def _make_fake_asana_request(task_sections=None, create_gid="new_task_gid", task_assignees=None):
    """task_sections: {existing_task_gid: current_section_name} — for
    GET /tasks/<gid> lookups on leads that already have an AsanaTaskGID.
    task_assignees: {existing_task_gid: assignee_email or None} — for
    the assignee-check GET made only when default_assignee_email is
    passed to sync_campaign_to_asana."""
    task_sections = task_sections or {}
    task_assignees = task_assignees or {}
    calls = []
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/tasks/" in url and "opt_fields=assignee.email" in url:
            task_gid = url.split("/tasks/")[1].split("?")[0]
            assignee_email = task_assignees.get(task_gid)
            assignee = {"email": assignee_email} if assignee_email else None
            return FakeAsanaResponse({"data": {"assignee": assignee}})
...
# =============================================================================
# Creator Tracker sheet sync — pure logic first (matching stage), then the
# full orchestration with the Sheets API mocked.
# =============================================================================

def _tracker_row(**overrides):
    row = {"Creator Email": "", "Creator": "", "Contact Status": "", "Last Contacted Date": ""}
    row.update(overrides)
    return row


# ---------- compute_lead_tracker_status ----------

def test_compute_lead_tracker_status_maps_every_asana_stage_to_its_own_label():
    cases = [
        ({}, outreach.TRACKER_STATUS_SOURCED),
        ({"IntroSentAt": "2026-08-01 09:00:00"}, outreach.TRACKER_STATUS_OUTREACHED),
        ({"IntroSentAt": "2026-08-01 09:00:00", "FollowUp1SentAt": "2026-08-05 09:00:00"},
         outreach.TRACKER_STATUS_FOLLOWUP),
        ({"IntroSentAt": "2026-08-01 09:00:00", "ReplyStatus": "Replied"},
         outreach.TRACKER_STATUS_NEGOTIATING),
        ({"ManualAsanaStage": "Rights Secured"}, outreach.TRACKER_STATUS_RIGHTS_SECURED),
        ({"ManualAsanaStage": "Declined / Dead"}, outreach.TRACKER_STATUS_DECLINED_DEAD),
    ]
    for lead_overrides, expected in cases:
        assert outreach.compute_lead_tracker_status(make_lead(**lead_overrides)) == expected


def test_compute_lead_tracker_status_shares_the_same_manual_override_as_asana():
    """The explicit request: one human decision (ManualAsanaStage),
    reflected consistently in BOTH Asana and the tracker sheet — not
    two independent overrides that could disagree with each other."""
    lead = make_lead(ManualAsanaStage="Rights Secured", ReplyStatus="Replied")
    assert outreach.compute_lead_tracker_status(lead) == outreach.TRACKER_STATUS_RIGHTS_SECURED


# ---------- sync_campaign_to_tracker_sheet (orchestration) ----------

class FakeTrackerWorksheet:
    ...
    def get_all_values(self):
        """Mirrors the real worksheet: row 1 is the header, then each
        record as a positional row. Values are looked up by the
        header's own order so a duplicate or blank header column is
        represented faithfully."""
        rows = [list(self._header)]
        for r in self._records:
            rows.append([str(r.get(col, "")) for col in self._header])
        return rows
...
def test_sync_campaign_to_tracker_sheet_disabled_is_a_noop():
    fake_sheets = FakeSheets([])
    campaign_cfg = {"tracker_sync": {"enabled": False}}
    result = outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws=None)
    assert result["skipped_disabled"] is True


def test_sync_campaign_to_tracker_sheet_updates_matching_row_only():
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky Rivera", "Contact Status": "Sourced"},
                 {"Creator Email": "someone_else@abc.com", "Creator": "Someone Else", "Contact Status": "Sourced"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    result = outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)

    assert result["updated"] == 1
    assert result["warnings"] == []
    # Row 2 (rocky) updated, row 3 (unrelated creator) never touched.
    assert tracker_ws.updated_cells == [(2, 3, "Outreached (Intro Sent)")]


def test_sync_campaign_to_tracker_sheet_never_reverses_rights_secured():
    """The explicit safety requirement — identical guarantee to Asana's
    manual-only-stage protection, applied here."""
    lead = make_lead(Email="rocky@abc.com", ReplyStatus="Replied", IntroSentAt="2026-08-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky Rivera", "Contact Status": "Rights Secured"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    ...
    assert tracker_ws.updated_cells == []  # never touched, despite computing to "Negotiating"


def test_sync_campaign_to_tracker_sheet_never_reverses_declined_dead():
    lead = make_lead(Email="rocky@abc.com", ReplyStatus="Replied")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky Rivera", "Contact Status": "Declined / Dead"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)

    assert tracker_ws.updated_cells == []


def test_sync_campaign_to_tracker_sheet_no_match_reported_as_warning():
    lead = make_lead(Email="nobody@abc.com", FirstName="Nobody")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"], records=[])
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    ...
    assert "nobody@abc.com" in result["warnings"][0]


def test_sync_campaign_to_tracker_sheet_ambiguous_match_reported_not_guessed():
    lead = make_lead(Email="", FirstName="Sam", LastName="Lee")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "", "Creator": "Sam Lee", "Contact Status": "Sourced"},
                 {"Creator Email": "", "Creator": "Sam Lee", "Contact Status": "Sourced"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
...
def test_sync_campaign_to_tracker_sheet_raises_clearly_when_no_contact_status_column():
    fake_sheets = FakeSheets([])
    tracker_ws = FakeTrackerWorksheet(header=["Creator Email", "Creator"], records=[])
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    with pytest.raises(RuntimeError, match="Contact Status"):
        outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)


def test_sync_campaign_to_tracker_sheet_unchanged_when_status_already_correct():
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
                  "Contact Status": "Outreached (Intro Sent)"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
...
def test_sync_campaign_to_tracker_sheet_writes_last_contacted_date_from_last_action_at():
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00",
                      LastActionAt="2026-08-01 09:15:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
                  "Contact Status": "Sourced", "Last Contacted Date": ""}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    ...
    assert result["updated"] == 1
    assert (2, 4, "2026-08-01") in tracker_ws.updated_cells  # column 4 = Last Contacted Date
    assert (2, 3, "Outreached (Intro Sent)") in tracker_ws.updated_cells  # column 3 = Contact Status


def test_sync_campaign_to_tracker_sheet_last_contacted_date_can_update_even_when_status_unchanged():
    """A second follow-up doesn't change the Contact Status LABEL (still
    'Follow up'), but the date should still advance to reflect the more
    recent contact."""
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00",
                      FollowUp1SentAt="2026-08-05 09:00:00", LastActionAt="2026-08-05 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
                  "Contact Status": "Follow up", "Last Contacted Date": "2026-08-01"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    result = outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)

    assert result["updated"] == 1  # counted as updated even though status text itself didn't change
    assert tracker_ws.updated_cells == [(2, 4, "2026-08-05")]  # only the date cell was written


# ---------- tracker: no automatic regression (same lead in two campaigns) ----------

def test_tracker_sync_never_regresses_status_automatically():
    """Found by end-to-end audit — the Creator Tracker equivalent of the
    Asana no-regression bug, and it was still live after that one was
    fixed. An older campaign (no reply on its own sheet, so it computes
    only "Outreached (Intro Sent)") sharing a tracker row with the
    active campaign would drag that row back from Negotiating on every
    single run, forever."""
    lead = make_lead(Email="d@x.com", IntroSentAt="2026-09-01 09:00:00",
                      LastActionAt="2026-09-01 09:00:00")
    ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "d@x.com", "Creator": "D", "Contact Status": "Negotiating",
                  "Last Contacted Date": "2026-09-10"}])
    result = outreach.sync_campaign_to_tracker_sheet(FakeSheets([lead]), {"tracker_sync": {"enabled": True}}, ws)
    assert result["updated"] == 0
    assert ws.updated_cells == []


def test_tracker_sync_forward_progress_still_works():
    lead = make_lead(Email="d@x.com", IntroSentAt="2026-09-01 09:00:00", ReplyStatus="Replied",
                      LastActionAt="2026-09-11 10:00:00")
    ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "d@x.com", "Creator": "D",
                  "Contact Status": "Outreached (Intro Sent)", "Last Contacted Date": "2026-09-01"}])
    outreach.sync_campaign_to_tracker_sheet(FakeSheets([lead]), {"tracker_sync": {"enabled": True}}, ws)
    header = ["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"]
    written = {header[col - 1]: val for _row, col, val in ws.updated_cells}
    assert written["Contact Status"] == "Negotiating"


def test_tracker_sync_explicit_override_can_still_move_backward():
    lead = make_lead(Email="d@x.com", ManualAsanaStage="Outreach Sent", LastActionAt="2026-09-01 09:00:00")
    ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "d@x.com", "Creator": "D", "Contact Status": "Negotiating",
                  "Last Contacted Date": "2026-09-10"}])
    ...
    header = ["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"]
    written = {header[col - 1]: val for _row, col, val in ws.updated_cells}
    assert written["Contact Status"] == "Outreached (Intro Sent)"


def test_tracker_sync_never_moves_last_contacted_date_backward():
    """Second bug found in the same audit: Last Contacted Date is a
    high-water mark, but an older campaign was overwriting a more
    recent contact date with its own earlier one."""
    lead = make_lead(Email="d@x.com", IntroSentAt="2026-09-01 09:00:00",
                      LastActionAt="2026-09-01 09:00:00")
    ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "d@x.com", "Creator": "D",
                  "Contact Status": "Outreached (Intro Sent)", "Last Contacted Date": "2026-09-10"}])
    outreach.sync_campaign_to_tracker_sheet(FakeSheets([lead]), {"tracker_sync": {"enabled": True}}, ws)
    header = ["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"]
    written = {header[col - 1]: val for _row, col, val in ws.updated_cells}
    assert "Last Contacted Date" not in written  # 09-01 never overwrites 09-10


def test_tracker_sync_non_iso_existing_date_still_gets_overwritten():
    """A hand-typed, unparseable date must not permanently block a real
    value from ever being written."""
    lead = make_lead(Email="d@x.com", IntroSentAt="2026-09-01 09:00:00",
                      LastActionAt="2026-09-11 10:00:00")
    ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "d@x.com", "Creator": "D",
                  "Contact Status": "Sourced", "Last Contacted Date": "Sept 1"}])
    outreach.sync_campaign_to_tracker_sheet(FakeSheets([lead]), {"tracker_sync": {"enabled": True}}, ws)
    header = ["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"]
    written = {header[col - 1]: val for _row, col, val in ws.updated_cells}
    assert written["Last Contacted Date"] == "2026-09-11"


# ---------- duplicate header columns in the Creator Tracker ----------

class RawValuesWorksheet:
    """A tracker worksheet fake driven by RAW values, so a header row
    with duplicate or blank column names can be represented exactly as
    the real sheet has it — which FakeTrackerWorksheet (dict-based)
    structurally cannot."""
...
def test_tracker_sync_survives_duplicate_header_columns():
    """The actual reported production failure: gspread's
    get_all_records() raises "the header row in the worksheet is not
    unique" if ANY header column name repeats, which aborted the entire
    tracker sync for every lead — even though every column the sync
    actually reads or writes was present and unambiguous. The Creator
    Tracker is a large hand-maintained sheet and genuinely does have
    repeated headers (here: "Notes" twice plus repeated blank trailing
    columns)."""
    values = [
        ["Creator Email", "Creator", "Contact Status", "Last Contacted Date",
         "Rights Duration", "Notes", "Notes", "", ""],
        ["d@x.com", "D", "Sourced", "", "", "n1", "n2", "", ""],
    ]
    lead = make_lead(Email="d@x.com", IntroSentAt="2026-09-01 09:00:00",
                      LastActionAt="2026-09-11 10:00:00")
    ws = RawValuesWorksheet(values)
    result = outreach.sync_campaign_to_tracker_sheet(
        FakeSheets([lead]), {"tracker_sync": {"enabled": True}}, ws)

    assert result["updated"] == 1
    written = {col: val for _row, col, val in ws.updated_cells}
    assert written[3] == "Outreached (Intro Sent)"   # physical Contact Status column
    assert written[4] == "2026-09-11"                # physical Last Contacted Date column


def test_build_tracker_records_takes_first_occurrence_of_a_duplicate():
    """Duplicates resolve to the FIRST occurrence, matching the column
    index header.index() computes for cell writes — so reads and writes
    can never disagree about which physical column they mean."""
    values = [["A", "B", "A", ""], ["1", "2", "3", "4"]]
    header, records = outreach._build_tracker_records_from_values(values)
    assert header == ["A", "B", "A", ""]
    assert records[0]["A"] == "1"   # first occurrence, not "3"
    assert records[0]["B"] == "2"
    assert "" not in records[0]     # blank header never becomes a key


def test_build_tracker_records_handles_short_rows_and_empty_sheet():
    header, records = outreach._build_tracker_records_from_values(
        [["A", "B", "C"], ["1"]])          # row shorter than the header
    assert records[0] == {"A": "1", "B": "", "C": ""}
    assert outreach._build_tracker_records_from_values([]) == ([], [])


def test_sync_campaign_to_tracker_sheet_never_writes_blank_last_contacted_date():
    """A lead that hasn't been contacted at all yet must never overwrite
    whatever's already in that cell with blank."""
    lead = make_lead(Email="rocky@abc.com")  # no LastActionAt at all
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
                  "Contact Status": "Sourced", "Last Contacted Date": "2026-07-01"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)

    assert tracker_ws.updated_cells == []  # neither column touched — status already matches, no date to write


def test_sync_campaign_to_tracker_sheet_never_calls_get_all_leads_when_leads_provided():
    """The actual reported production issue, tested directly against
    the real function (not a mock standing in for it) — passing leads
    explicitly must genuinely skip the internal sheets.get_all_leads()
    call, not just accept the parameter and ignore it."""
    lead = make_lead(Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky", "Contact Status": "Sourced"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws,
                                             leads=fake_sheets.get_all_leads())

    # The one explicit call above to build the `leads` argument is the
    # ONLY call — the function itself must add zero more.
    assert fake_sheets.get_all_leads_call_count == 1


def test_sync_campaign_to_tracker_sheet_still_reads_fresh_when_leads_omitted():
    """Backward compatible — omitting leads entirely must still read
    ...
    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)  # leads omitted

    assert fake_sheets.get_all_leads_call_count == 1


def test_sync_campaign_to_tracker_sheet_manual_status_also_freezes_last_contacted_date():
    """The whole row is frozen once manually finalized — not just the
    status column."""
    lead = make_lead(Email="rocky@abc.com", ReplyStatus="Replied", LastActionAt="2026-09-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky",
                  "Contact Status": "Rights Secured", "Last Contacted Date": "2026-08-01"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)

    assert tracker_ws.updated_cells == []  # Last Contacted Date NOT bumped to 2026-09-01


def test_sync_campaign_to_tracker_sheet_missing_last_contacted_column_raises():
    fake_sheets = FakeSheets([])
    tracker_ws = FakeTrackerWorksheet(header=["Creator Email", "Creator", "Contact Status"], records=[])
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    with pytest.raises(RuntimeError, match="Last Contacted Date"):
        outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)


def test_sync_campaign_to_tracker_sheet_missing_rights_duration_column_raises():
    fake_sheets = FakeSheets([])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date"], records=[])
    campaign_cfg = {"tracker_sync": {"enabled": True}}

    with pytest.raises(RuntimeError, match="Rights Duration"):
        outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws)


# ---------- Asana -> Tracker reverse sync (manual decisions made directly in Asana) ----------

def test_sync_campaign_to_tracker_sheet_uses_asana_live_section_when_sheet_override_blank():
    """The actual feature request: a task dragged to Rights Secured
    directly in Asana, with NO corresponding ManualAsanaStage typed
    into the Sheet, must still reach the tracker."""
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00")  # would otherwise compute to Outreached
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky", "Contact Status": "Follow up"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    asana_state = {"rocky@abc.com": {"section": "Rights Secured", "rights_expiration": None}}

    result = outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws,
                                                       asana_manual_state_by_email=asana_state)

    assert result["updated"] == 1
    assert (2, 3, "Rights Secured") in tracker_ws.updated_cells


def test_sync_campaign_to_tracker_sheet_writes_rights_duration_from_asana_rights_expiration():
    lead = make_lead(Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
    ...
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    asana_state = {"rocky@abc.com": {"section": "Rights Secured", "rights_expiration": "2027-03-04"}}

    result = outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws,
                                                       asana_manual_state_by_email=asana_state)

    assert result["updated"] == 1
    assert (2, 5, "2027-03-04") in tracker_ws.updated_cells  # column 5 = Rights Duration


def test_sync_campaign_to_tracker_sheet_sheet_side_manual_override_wins_over_asana_live_state():
    """An explicit ManualAsanaStage on the Sheet must never be silently
    overridden by whatever Asana's live section happens to say —
    e.g. someone reverted a decision on the Sheet but the Asana task
    hasn't been moved back yet."""
    lead = make_lead(Email="rocky@abc.com", ManualAsanaStage="Declined / Dead")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky", "Contact Status": "Sourced"}],
    ...
    assert (2, 3, "Declined / Dead") in tracker_ws.updated_cells
    assert (2, 3, "Rights Secured") not in tracker_ws.updated_cells


def test_sync_campaign_to_tracker_sheet_asana_non_manual_section_never_overrides_status():
    """Asana's live state only matters here when the section is
    genuinely Rights Secured / Declined — an ordinary section like
    Follow-up must never be treated as an override."""
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
    ...
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    asana_state = {"rocky@abc.com": {"section": "Follow-up", "rights_expiration": None}}

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws,
                                             asana_manual_state_by_email=asana_state)

    assert (2, 3, "Outreached (Intro Sent)") in tracker_ws.updated_cells
    assert not any(c[2] == "Follow-up" for c in tracker_ws.updated_cells)


def test_sync_campaign_to_tracker_sheet_no_asana_state_provided_is_unaffected():
    """Backward compatible — every existing caller that doesn't pass
    asana_manual_state_by_email at all must behave exactly as before."""
    lead = make_lead(Email="rocky@abc.com", IntroSentAt="2026-08-01 09:00:00")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
...
def test_sync_campaign_to_tracker_sheet_never_writes_blank_rights_duration():
    lead = make_lead(Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    tracker_ws = FakeTrackerWorksheet(
        header=["Creator Email", "Creator", "Contact Status", "Last Contacted Date", "Rights Duration"],
        records=[{"Creator Email": "rocky@abc.com", "Creator": "Rocky", "Contact Status": "Sourced",
                  "Rights Duration": "2027-01-01"}],
    )
    campaign_cfg = {"tracker_sync": {"enabled": True}}
    asana_state = {"rocky@abc.com": {"section": "Sourced", "rights_expiration": None}}  # no expiration set

    outreach.sync_campaign_to_tracker_sheet(fake_sheets, campaign_cfg, tracker_ws,
                                             asana_manual_state_by_email=asana_state)

    assert not any(c[1] == 5 for c in tracker_ws.updated_cells)  # Rights Duration cell never touched


# ---------- _connect_tracker_worksheet ----------

def test_connect_tracker_worksheet_accepts_a_tab_name(monkeypatch):
    fake_ws = object()

    class FakeSpreadsheet:
        def worksheet(self, title):
            assert title == "Duderobe"
            return fake_ws
    ...
    ws = outreach._connect_tracker_worksheet("fake_sheet_id", "Duderobe")
    assert ws is not None


def test_connect_tracker_worksheet_accepts_a_numeric_gid(monkeypatch):
    """The actual reported confusion: a spreadsheet's own document
    title ('Content Tracker') is a different thing from a specific
    tab's name or its numeric gid — supporting a gid directly avoids
    needing to know the tab's exact name at all, and is more robust
    since a gid never changes even if the tab gets renamed later."""
    fake_ws = object()
    ...
    class FakeSpreadsheet:
        ...
        def worksheet(self, title):
            raise AssertionError("should never call worksheet() by name when given a numeric gid")
    ...
    ws = outreach._connect_tracker_worksheet("fake_sheet_id", "29908264")
    assert ws is not None


def test_connect_tracker_worksheet_error_explains_document_title_vs_tab_name(monkeypatch):
    class FakeSpreadsheet:
        def worksheet(self, title):
            raise FakeGspreadModule.exceptions.WorksheetNotFound()
    ...
    with pytest.raises(RuntimeError, match="NOT the spreadsheet document's own title"):
        outreach._connect_tracker_worksheet("fake_sheet_id", "Content Tracker")


# ---------- build_tracker_row_index / find_tracker_row_for_lead ----------

def _by_row(rows):
    """Test helper — builds the {row_number: row_dict} lookup
    find_tracker_row_for_lead needs for disambiguation, from a plain
    list of _tracker_row() dicts."""
    return {r["_row"]: r for r in rows}


def test_find_tracker_row_for_lead_matches_by_email_first():
    rows = [_tracker_row(**{"Creator Email": "rocky@abc.com", "_row": 2}),
            _tracker_row(**{"Creator": "Rocky Rivera", "_row": 3})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="rocky@abc.com", FirstName="Rocky", LastName="Rivera")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 2, "matched_by": "email", "ambiguous": False}


def test_find_tracker_row_for_lead_falls_back_to_name_when_no_email_match():
    rows = [_tracker_row(**{"Creator": "Rocky Rivera", "_row": 5})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="", FirstName="Rocky", LastName="Rivera")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 5, "matched_by": "name", "ambiguous": False}


def test_find_tracker_row_for_lead_matches_by_handle_when_only_handle_is_in_the_tracker():
    rows = [_tracker_row(**{"Creator": "@rockyriv", "_row": 7})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="", FirstName="Rocky", LastName="Rivera", Creator="@rockyriv")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 7, "matched_by": "name", "ambiguous": False}


def test_find_tracker_row_for_lead_ambiguous_name_match_never_guesses_without_disambiguating_data():
    """The actual safety property this whole design leans on — a name
    matching MORE than one row must never be silently resolved by
    picking one when there's no Video File / Refunnel Link data to
    disambiguate with; it has to be reported instead."""
    rows = [_tracker_row(**{"Creator": "Sam Lee", "_row": 10}),
            _tracker_row(**{"Creator": "Sam Lee", "_row": 400})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="", FirstName="Sam", LastName="Lee")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": None, "matched_by": None, "ambiguous": True}


def test_find_tracker_row_for_lead_no_match_at_all():
    email_index, name_index = outreach.build_tracker_row_index([])
    lead = make_lead(Email="nobody@abc.com", FirstName="Nobody")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, {})
    assert match == {"row": None, "matched_by": None, "ambiguous": False}


def test_find_tracker_row_for_lead_handles_integer_type_safely():
    """Same class of Sheet-typing risk as AsanaTaskGID — a numeric-
    looking Email or Creator field must never crash this."""
    rows = [_tracker_row(**{"Creator Email": "12345", "_row": 2})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email=12345, FirstName="")
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match["row"] == 2


# ---------- The actual reported case: same creator, multiple tracker rows ----------

def test_find_tracker_row_for_lead_disambiguates_duplicate_email_by_video_file():
    """The exact reported scenario: same creator (same email) appears
    on more than one tracker row — one per video — and Video File on
    the lead's own side is what narrows it down to the right one."""
    rows = [_tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 2, "Video File": "video1.mp4"}),
            _tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 3, "Video File": "video2.mp4"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="jjung95@gmail.com", **{"Video File": "video2.mp4"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 3, "matched_by": "email+video/refunnel", "ambiguous": False}


def test_find_tracker_row_for_lead_disambiguates_by_refunnel_link_when_video_file_blank():
    rows = [_tracker_row(**{"Creator Email": "dave@abc.com", "_row": 2, "Refunnel Link": "link-a"}),
            _tracker_row(**{"Creator Email": "dave@abc.com", "_row": 3, "Refunnel Link": "link-b"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="dave@abc.com", **{"Refunnel Link": "link-b"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 3, "matched_by": "email+video/refunnel", "ambiguous": False}


def test_find_tracker_row_for_lead_disambiguates_duplicate_name_match_too():
    """The disambiguation applies to name-based collisions exactly the
    same way as email-based ones — the mechanism doesn't care which
    identifier found the candidates."""
    rows = [_tracker_row(**{"Creator": "Sam Lee", "_row": 10, "Video File": "video1.mp4"}),
            _tracker_row(**{"Creator": "Sam Lee", "_row": 400, "Video File": "video2.mp4"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="", FirstName="Sam", LastName="Lee", **{"Video File": "video1.mp4"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 10, "matched_by": "name+video/refunnel", "ambiguous": False}


def test_find_tracker_row_for_lead_still_ambiguous_when_no_video_file_matches_any_candidate():
    rows = [_tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 2, "Video File": "video1.mp4"}),
            _tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 3, "Video File": "video2.mp4"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="jjung95@gmail.com", **{"Video File": "some_other_video.mp4"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": None, "matched_by": None, "ambiguous": True}


def test_find_tracker_row_for_lead_still_ambiguous_when_video_file_matches_more_than_one_candidate():
    """Two rows accidentally sharing the same Video File value must
    still be treated as genuinely ambiguous, not resolved by picking
    whichever one comes first."""
    rows = [_tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 2, "Video File": "same.mp4"}),
            _tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 3, "Video File": "same.mp4"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="jjung95@gmail.com", **{"Video File": "same.mp4"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": None, "matched_by": None, "ambiguous": True}


def test_find_tracker_row_for_lead_no_disambiguating_data_on_lead_stays_ambiguous():
    rows = [_tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 2, "Video File": "video1.mp4"}),
            _tracker_row(**{"Creator Email": "jjung95@gmail.com", "_row": 3, "Video File": "video2.mp4"})]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="jjung95@gmail.com")  # no Video File / Refunnel Link at all
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": None, "matched_by": None, "ambiguous": True}


# ---------- _extract_tiktok_video_id / URL-format-mismatch disambiguation ----------

def test_extract_tiktok_video_id_from_long_form_video_file():
    url = "https://www.tiktok.com/@allidoislyn/video/7670520946247077151"
    assert outreach._extract_tiktok_video_id(url) == "7670520946247077151"


def test_extract_tiktok_video_id_from_refunnel_link():
    url = "https://collections.refunnel.com/public-media?shop_id=813299&media_id=tk_7671018298112347406"
    assert outreach._extract_tiktok_video_id(url) == "7671018298112347406"


def test_extract_tiktok_video_id_returns_none_for_short_share_link():
    """The actual reported root cause — TikTok's short, opaque share
    link format doesn't contain the numeric video ID at all, which is
    exactly why raw string comparison alone could never resolve this
    case no matter how correct the underlying data was."""
    assert outreach._extract_tiktok_video_id("https://www.tiktok.com/t/ZP8GtUjfh/") is None


def test_disambiguate_tracker_candidates_resolves_short_vs_long_link_mismatch():
    """The exact real-world scenario found by inspecting the actual
    Asana project directly: the lead's own Video File is TikTok's
    short share link (can't be compared as a raw string against a
    long-form URL on the tracker side), but its Refunnel Link embeds
    the same underlying numeric video ID as tk_<id> — extracting and
    comparing that ID is what actually resolves this, where the
    original exact-string-only comparison could not."""
    lead = make_lead(Email="mizzsandy928@gmail.com",
                      **{"Video File": "https://www.tiktok.com/t/ZP8GtUjfh/",
                         "Refunnel Link": "https://collections.refunnel.com/public-media?"
                                          "shop_id=813299&media_id=tk_7661826301912714510"})
    tracker_records_by_row = {
        2: {"Video File": "https://www.tiktok.com/@sandycheeks92203/video/7661826301912714510",
            "Refunnel Link": ""},
        3: {"Video File": "https://www.tiktok.com/@someone_else/video/9999999999999999999",
            "Refunnel Link": ""},
    }
    result = outreach._disambiguate_tracker_candidates(lead, [2, 3], tracker_records_by_row)
    assert result == 2


def test_disambiguate_tracker_candidates_still_ambiguous_when_video_ids_dont_match_anything():
    lead = make_lead(Email="rocky@abc.com",
                      **{"Video File": "https://www.tiktok.com/t/ZP8GtUjfh/",
                         "Refunnel Link": "https://collections.refunnel.com/public-media?"
                                          "media_id=tk_1111111111111111111"})
    tracker_records_by_row = {
        2: {"Video File": "https://www.tiktok.com/@a/video/2222222222222222222", "Refunnel Link": ""},
        3: {"Video File": "https://www.tiktok.com/@b/video/3333333333333333333", "Refunnel Link": ""},
    }
    result = outreach._disambiguate_tracker_candidates(lead, [2, 3], tracker_records_by_row)
    assert result is None


def test_disambiguate_tracker_candidates_exact_string_match_still_tried_first():
    """The new video-ID fallback must never interfere with the
    original, still-valid exact-string comparison — a plain match on
    identical raw URLs must keep working exactly as before."""
    lead = make_lead(Email="rocky@abc.com", **{"Video File": "video1.mp4"})
    tracker_records_by_row = {
        2: {"Video File": "video1.mp4", "Refunnel Link": ""},
        3: {"Video File": "video2.mp4", "Refunnel Link": ""},
    }
    result = outreach._disambiguate_tracker_candidates(lead, [2, 3], tracker_records_by_row)
    assert result == 2


def test_find_tracker_row_for_lead_end_to_end_resolves_the_reported_stewart_lyn_style_case():
    """Full end-to-end reproduction through find_tracker_row_for_lead
    itself (not just the disambiguation helper in isolation) — the
    exact class of case reported: an email matching more than one
    tracker row, resolved correctly via the Refunnel Link's embedded
    video ID despite Video File being a short link on one side."""
    rows = [
        _tracker_row(**{"Creator Email": "mizzsandy928@gmail.com", "_row": 2,
                         "Video File": "https://www.tiktok.com/@sandycheeks92203/video/7661826301912714510"}),
        _tracker_row(**{"Creator Email": "mizzsandy928@gmail.com", "_row": 3,
                         "Video File": "https://www.tiktok.com/@sandycheeks92203/video/9999999999999999999"}),
    ]
    email_index, name_index = outreach.build_tracker_row_index(rows)
    lead = make_lead(Email="mizzsandy928@gmail.com",
                      **{"Video File": "https://www.tiktok.com/t/ZP8GtUjfh/",
                         "Refunnel Link": "https://collections.refunnel.com/public-media?"
                                          "shop_id=813299&media_id=tk_7661826301912714510"})
    match = outreach.find_tracker_row_for_lead(lead, email_index, name_index, _by_row(rows))
    assert match == {"row": 2, "matched_by": "email+video/refunnel", "ambiguous": False}


def test_build_tracker_row_index_detects_email_collisions_too():
    """The actual latent bug fixed alongside this feature: an earlier
    version let a later row silently overwrite an earlier one sharing
    the same email in the index, meaning this collision was never even
    detected. Now both entries are tracked."""
    rows = [_tracker_row(**{"Creator Email": "dup@abc.com", "_row": 2}),
            _tracker_row(**{"Creator Email": "dup@abc.com", "_row": 5})]
    email_index, _ = outreach.build_tracker_row_index(rows)
    assert sorted(email_index["dup@abc.com"]) == [2, 5]
...
def test_sync_campaign_to_asana_sets_assignee_on_new_task_creation(monkeypatch):
    """The actual feature request — every new task should get the
    configured assignee set at creation."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    fake_request, calls = _make_fake_asana_request()
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    outreach.sync_campaign_to_asana(fake_sheets, campaign_cfg, api_key="fake-key",
                                     default_assignee_email="harish@kelson.agency")

    create_calls = [c for c in calls if c[0] == "POST" and c[1].endswith("/tasks")]
    assert len(create_calls) == 1
    assert create_calls[0][2]["data"]["assignee"] == "harish@kelson.agency"


def test_sync_campaign_to_asana_backfills_assignee_on_existing_unassigned_task(monkeypatch):
    """The other half of the request — "for the existing" tasks too,
    not just future ones."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="existing_gid_123")
    fake_sheets = FakeSheets([lead])
    fake_request, calls = _make_fake_asana_request(
        task_sections={"existing_gid_123": "Outreach Sent"},
        task_assignees={"existing_gid_123": None},  # currently unassigned
    )
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    assert update_calls[0][2]["data"]["assignee"] == "harish@kelson.agency"


def test_sync_campaign_to_asana_never_overwrites_a_different_existing_assignee(monkeypatch):
    """A human deliberately assigning a task to someone else must never
    be silently reversed by a sync run — only a genuinely blank
    assignee gets filled in."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="existing_gid_123")
    fake_sheets = FakeSheets([lead])
    fake_request, calls = _make_fake_asana_request(
        task_sections={"existing_gid_123": "Outreach Sent"},
        task_assignees={"existing_gid_123": "someone.else@kelson.agency"},
    )
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    update_calls = [c for c in calls if c[0] == "PUT" and "/tasks/existing_gid_123" in c[1]]
    assert len(update_calls) == 1
    assert "assignee" not in update_calls[0][2]["data"]


def test_sync_campaign_to_asana_no_assignee_check_at_all_when_not_configured(monkeypatch):
    """When default_assignee_email isn't set, the assignee-check GET
    should never even be made — no wasted API calls for a feature
    ...
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    outreach.sync_campaign_to_asana(fake_sheets, campaign_cfg, api_key="fake-key")  # no default_assignee_email

    assignee_check_calls = [c for c in calls if c[0] == "GET" and "opt_fields=assignee.email" in c[1]]
    assert assignee_check_calls == []


def test_sync_campaign_to_asana_never_calls_get_all_leads_when_leads_provided(monkeypatch):
    """Direct test against the real function — not a mock standing in
    for it — proving passing leads explicitly genuinely skips the
    internal sheets.get_all_leads() call. A contributor to hitting
    Google's Sheets API per-minute read quota when both Asana sync and
    Tracker sync are enabled for the same campaign."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    fake_request, _ = _make_fake_asana_request()
    monkeypatch.setattr(outreach.requests, "request", fake_request)
...
def test_sync_campaign_to_asana_still_reads_fresh_when_leads_omitted(monkeypatch):
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com")
    fake_sheets = FakeSheets([lead])
    fake_request, _ = _make_fake_asana_request()
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    outreach.sync_campaign_to_asana(fake_sheets, campaign_cfg, api_key="fake-key")  # leads omitted

    assert fake_sheets.get_all_leads_call_count == 1


def test_collect_asana_manual_state_for_leads_never_calls_get_all_leads_when_leads_provided(monkeypatch):
    lead = make_lead(Email="rocky@abc.com", AsanaTaskGID="gid1")
    fake_sheets = FakeSheets([lead])
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if "/projects?" in url:
            return FakeAsanaResponse({"data": [{"name": "Creator Outreach", "gid": "proj_1"}]})
        return FakeAsanaResponse({"data": {"memberships": [], "custom_fields": []}})

    import unittest.mock
    with unittest.mock.patch.object(outreach.requests, "request", fake_request):
        outreach.collect_asana_manual_state_for_leads(fake_sheets, campaign_cfg, api_key="fake-key",
                                                        leads=fake_sheets.get_all_leads())

    assert fake_sheets.get_all_leads_call_count == 1


# ---------- asana_get_task_manual_state ----------

def test_asana_get_task_manual_state_reads_section_and_rights_expiration(monkeypatch):
    def fake_request(method, url, headers=None, timeout=None, json=None):
        assert method == "GET"
        assert "custom_fields.name" in url and "custom_fields.date_value" in url
        return FakeAsanaResponse({"data": {
            "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Rights Secured"}}],
            "custom_fields": [
                {"name": "Content Score", "date_value": None},
                {"name": "Rights Expiration", "date_value": {"date": "2027-03-04"}},
            ],
        }})

    monkeypatch.setattr(outreach.requests, "request", fake_request)
    state = outreach.asana_get_task_manual_state("task_1", "proj_1", api_key="fake-key")
    assert state == {"section": "Rights Secured", "rights_expiration": "2027-03-04"}


def test_asana_get_task_manual_state_no_rights_expiration_field_set(monkeypatch):
    def fake_request(method, url, headers=None, timeout=None, json=None):
        return FakeAsanaResponse({"data": {
            "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Outreach Sent"}}],
            "custom_fields": [{"name": "Rights Expiration", "date_value": None}],
        }})

    monkeypatch.setattr(outreach.requests, "request", fake_request)
    state = outreach.asana_get_task_manual_state("task_1", "proj_1", api_key="fake-key")
    assert state == {"section": "Outreach Sent", "rights_expiration": None}


def test_asana_get_task_manual_state_no_matching_project_membership(monkeypatch):
    def fake_request(method, url, headers=None, timeout=None, json=None):
        return FakeAsanaResponse({"data": {"memberships": [], "custom_fields": []}})

    monkeypatch.setattr(outreach.requests, "request", fake_request)
    state = outreach.asana_get_task_manual_state("task_1", "proj_1", api_key="fake-key")
    assert state == {"section": None, "rights_expiration": None}


# ---------- collect_asana_manual_state_for_leads ----------

def test_collect_asana_manual_state_for_leads_returns_empty_when_asana_not_enabled():
    fake_sheets = FakeSheets([make_lead(Email="rocky@abc.com", AsanaTaskGID="gid1")])
    campaign_cfg = {"asana": {"enabled": False}}
    result = outreach.collect_asana_manual_state_for_leads(fake_sheets, campaign_cfg, api_key="fake-key")
    assert result == {}


def test_collect_asana_manual_state_for_leads_skips_leads_without_task_gid(monkeypatch):
    fake_sheets = FakeSheets([make_lead(Email="rocky@abc.com", AsanaTaskGID="")])
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    fake_request, calls = _make_fake_asana_request()
    monkeypatch.setattr(outreach.requests, "request", fake_request)
    ...
    assert result == {}
    manual_state_calls = [c for c in calls if "custom_fields.date_value" in c[1]]
    assert manual_state_calls == []


def test_collect_asana_manual_state_for_leads_keyed_by_lowercased_email(monkeypatch):
    fake_sheets = FakeSheets([make_lead(Email="Rocky@ABC.com", AsanaTaskGID="gid1")])
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if "/projects?" in url:
            return FakeAsanaResponse({"data": [{"name": "Creator Outreach", "gid": "proj_1"}]})
        if "custom_fields.date_value" in url:
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Rights Secured"}}],
                "custom_fields": [],
            }})
    ...
    assert "rocky@abc.com" in result
    assert result["rocky@abc.com"]["section"] == "Rights Secured"


def test_collect_asana_manual_state_for_leads_one_lead_failure_does_not_lose_others(monkeypatch):
    leads = [make_lead(Email="fails@abc.com", AsanaTaskGID="bad_gid"),
             make_lead(Email="works@abc.com", AsanaTaskGID="good_gid")]
    fake_sheets = FakeSheets(leads)
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Creator Outreach"}}
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if "/projects?" in url:
            return FakeAsanaResponse({"data": [{"name": "Creator Outreach", "gid": "proj_1"}]})
        if "/tasks/bad_gid" in url:
            return FakeAsanaResponse({}, status_code=404)
        if "/tasks/good_gid" in url:
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Rights Secured"}}],
                "custom_fields": [],
            }})
    ...
    assert "fails@abc.com" not in result
    assert result["works@abc.com"]["section"] == "Rights Secured"


def test_collect_asana_manual_state_for_leads_returns_empty_when_project_not_found(monkeypatch):
    fake_sheets = FakeSheets([make_lead(Email="rocky@abc.com", AsanaTaskGID="gid1")])
    campaign_cfg = {"asana": {"enabled": True, "project_name": "Nonexistent Project"}}

    def fake_request(method, url, headers=None, timeout=None, json=None):
        if url.endswith("/workspaces"):
            return FakeAsanaResponse({"data": [{"name": "Kelson Agency", "gid": "workspace_1"}]})
        if "/projects?" in url:
            return FakeAsanaResponse({"data": [{"name": "Some Other Project", "gid": "proj_x"}]})
        raise AssertionError(f"Unexpected call: {url}")
...
def test_sync_campaign_to_asana_update_raises_clearly_when_target_section_missing(monkeypatch):
    """The actual reported production bug: a move failing to find its
    target section during an UPDATE used to be silently skipped with
    zero error — the sync would report 'updated: 1' while the task
    never actually moved, indistinguishable from a genuine no-op. Now
    it's a real, visible error, matching what the create path already
    did (see the very similar existing test for create, below)."""
    lead = make_lead(_row=2, LeadID="L1", Email="rocky@abc.com", AsanaTaskGID="existing_gid_123",
                      ManualAsanaStage="Negotiating")
    fake_sheets = FakeSheets([lead])
    ...
    def fake_request(method, url, headers=None, timeout=None, json=None):
        ...
        if method == "GET" and "/projects/proj_1?" in url:
            # Deliberately missing "Negotiating" — simulating the project's
            # real section being named something slightly different.
            return FakeAsanaResponse({"data": {
                "sections": [{"name": "Outreach Sent", "gid": "sec_outreach"}],
                "custom_field_settings": [],
            }})
        if method == "GET" and "/tasks/existing_gid_123" in url and "opt_fields=memberships" in url:
            return FakeAsanaResponse({"data": {
                "memberships": [{"project": {"gid": "proj_1"}, "section": {"name": "Outreach Sent"}}],
            }})
        if method == "PUT" and "/tasks/existing_gid_123" in url:
            return FakeAsanaResponse({"data": {}})
    ...
    assert result["updated"] == 0  # never silently counted as a successful update
    assert len(result["errors"]) == 1
    assert result["errors"][0]["email"] == "rocky@abc.com"
    assert "Negotiating" in result["errors"][0]["error"]
    assert "Outreach Sent" in result["errors"][0]["error"]  # available sections listed, for easy diagnosis
...
def test_cmd_sync_asana_all_syncs_only_enabled_campaigns(monkeypatch, capsys):
    ...
    def fake_sync(sheets, campaign_cfg, api_key, default_assignee_email=None, leads=None):
...
def test_cmd_sync_asana_all_one_campaign_failure_does_not_block_others(monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Foo" in out and "Asana sync failed entirely" in out
    assert "Bar (Asana): created 2" in out  # Bar still succeeded despite Foo's failure


def test_cmd_sync_asana_all_nothing_enabled_is_a_quiet_no_op(monkeypatch, capsys):
    """No token being set is no longer, by itself, an error — it only
    matters if some campaign actually has Asana or Tracker sync
    enabled. An environment where nothing is configured yet should
    just report that plainly, not fail."""
    monkeypatch.delenv("ASANA_ACCESS_TOKEN", raising=False)
    monkeypatch.delenv("CREATOR_TRACKER_SHEET_ID", raising=False)
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {"asana": {"enabled": False}})

    outreach.cmd_sync_asana_all(argparse.Namespace())  # must NOT raise SystemExit

    out = capsys.readouterr().out
    assert "nothing to do" in out


def test_cmd_sync_asana_all_asana_enabled_but_token_missing_still_exits_nonzero(monkeypatch, capsys):
    monkeypatch.delenv("ASANA_ACCESS_TOKEN", raising=False)
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign",
                         lambda name, **kw: {"asana": {"enabled": True, "project_name": "X"}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
...
def test_cmd_sync_asana_all_runs_tracker_sync_independently_of_asana(monkeypatch, capsys):
    """The actual point of the independent design — a campaign with
    ONLY tracker_sync enabled (no Asana at all) must still sync
    correctly, with no Asana token required."""
    monkeypatch.delenv("ASANA_ACCESS_TOKEN", raising=False)
    monkeypatch.setenv("CREATOR_TRACKER_SHEET_ID", "tracker_sheet_123")
    monkeypatch.setenv("CREATOR_TRACKER_WORKSHEET_NAME", "Master Tab")
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign",
                         lambda name, **kw: {"tracker_sync": {"enabled": True}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "_connect_tracker_worksheet", lambda sid, name: "fake_ws")
    monkeypatch.setattr(outreach, "sync_campaign_to_tracker_sheet",
                         lambda sheets, cfg, ws, **kw: {"updated": 3, "unchanged": 1, "warnings": []})

    outreach.cmd_sync_asana_all(argparse.Namespace())  # must NOT raise SystemExit

    out = capsys.readouterr().out
    assert "Foo (Creator Tracker): updated 3, unchanged 1." in out


def test_cmd_sync_asana_all_tracker_connection_reused_across_campaigns(monkeypatch, capsys):
    """The Creator Tracker sheet is the SAME shared sheet for every
    campaign — connecting to it once and reusing that connection across
    every campaign's sync avoids repeatedly re-reading a 3000+ row
    sheet once per campaign in a single run."""
    monkeypatch.setenv("CREATOR_TRACKER_SHEET_ID", "tracker_sheet_123")
    monkeypatch.setenv("CREATOR_TRACKER_WORKSHEET_NAME", "Master Tab")
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo", "Bar"])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {"tracker_sync": {"enabled": True}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    connect_calls = []
    monkeypatch.setattr(outreach, "_connect_tracker_worksheet",
                         lambda sid, name: connect_calls.append(1) or "fake_ws")
    monkeypatch.setattr(outreach, "sync_campaign_to_tracker_sheet",
                         lambda sheets, cfg, ws, **kw: {"updated": 0, "unchanged": 0, "warnings": []})
...
def test_cmd_sync_asana_all_reads_the_master_sheet_exactly_once_per_campaign(monkeypatch):
    """The actual reported production issue: with BOTH Asana and
    Tracker sync enabled for a campaign, the Master Sheet was
    previously read once per function (up to 3 times total) — a real,
    confirmed contributor to hitting Google's Sheets API per-minute
    read quota. It must now be read exactly once per campaign and
    reused across all three functions."""
    monkeypatch.setenv("ASANA_ACCESS_TOKEN", "fake-key")
    monkeypatch.setenv("CREATOR_TRACKER_SHEET_ID", "tracker_sheet_123")
    monkeypatch.setenv("CREATOR_TRACKER_WORKSHEET_NAME", "Master Tab")
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {
        "asana": {"enabled": True, "project_name": "Creator Outreach"},
        "tracker_sync": {"enabled": True},
    })
    fake_sheets = FakeSheets([make_lead(Email="rocky@abc.com")])
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: fake_sheets)
    monkeypatch.setattr(outreach, "sync_campaign_to_asana",
                         lambda sheets, cfg, api_key, **kw: {"created": 0, "updated": 0,
                                                              "skipped_no_email": 0, "errors": []})
    monkeypatch.setattr(outreach, "_connect_tracker_worksheet", lambda sid, name: "fake_ws")
    monkeypatch.setattr(outreach, "collect_asana_manual_state_for_leads", lambda sheets, cfg, key, **kw: {})
    monkeypatch.setattr(outreach, "sync_campaign_to_tracker_sheet",
                         lambda sheets, cfg, ws, **kw: {"updated": 0, "unchanged": 0, "warnings": []})
...
def test_cmd_sync_asana_all_the_same_leads_object_reaches_all_three_functions(monkeypatch):
    """Not just 'read once' — the SAME fetched list must actually reach
    every function, not just be fetched once and then ignored while
    each function still calls get_all_leads() itself."""
    monkeypatch.setenv("ASANA_ACCESS_TOKEN", "fake-key")
    monkeypatch.setenv("CREATOR_TRACKER_SHEET_ID", "tracker_sheet_123")
    monkeypatch.setenv("CREATOR_TRACKER_WORKSHEET_NAME", "Master Tab")
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    ...
    def fake_asana_sync(sheets, cfg, api_key, leads=None, **kw):
        received_leads["asana"] = leads
        return {"created": 0, "updated": 0, "skipped_no_email": 0, "errors": []}

    def fake_collect_state(sheets, cfg, key, leads=None, **kw):
        received_leads["collect"] = leads
        return {}

    def fake_tracker_sync(sheets, cfg, ws, leads=None, **kw):
        received_leads["tracker"] = leads
        return {"updated": 0, "unchanged": 0, "warnings": []}

    monkeypatch.setattr(outreach, "sync_campaign_to_asana", fake_asana_sync)
    monkeypatch.setattr(outreach, "_connect_tracker_worksheet", lambda sid, name: "fake_ws")
    monkeypatch.setattr(outreach, "collect_asana_manual_state_for_leads", fake_collect_state)
    monkeypatch.setattr(outreach, "sync_campaign_to_tracker_sheet", fake_tracker_sync)

    outreach.cmd_sync_asana_all(argparse.Namespace())

    assert fake_sheets.get_all_leads_call_count == 1
    assert received_leads["asana"] is received_leads["collect"] is received_leads["tracker"]
    assert received_leads["asana"][0]["Email"] == "rocky@abc.com"


def test_cmd_sync_asana_all_passes_asana_manual_state_through_to_tracker_sync(monkeypatch):
    """The actual end-to-end feature: with BOTH Asana and Tracker sync
    enabled for a campaign, a decision made directly in Asana must
    genuinely reach sync_campaign_to_tracker_sheet, not just exist as
    an unused, dead code path."""
    monkeypatch.setenv("ASANA_ACCESS_TOKEN", "fake-key")
    monkeypatch.setenv("CREATOR_TRACKER_SHEET_ID", "tracker_sheet_123")
    monkeypatch.setenv("CREATOR_TRACKER_WORKSHEET_NAME", "Master Tab")
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    ...
    fake_manual_state = {"rocky@abc.com": {"section": "Rights Secured", "rights_expiration": "2027-03-04"}}
    monkeypatch.setattr(outreach, "collect_asana_manual_state_for_leads",
                         lambda sheets, cfg, key, **kw: fake_manual_state)

    captured_state = {}

    def fake_tracker_sync(sheets, cfg, ws, asana_manual_state_by_email=None, leads=None):
        captured_state["received"] = asana_manual_state_by_email
        return {"updated": 0, "unchanged": 0, "warnings": []}
    ...
    assert captured_state["received"] == fake_manual_state
...
def test_auto_send_all_skips_non_running_campaigns(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo", "Bar"])
    monkeypatch.setattr(outreach, "get_campaign",
                         lambda name, **kw: {"status": "paused" if name == "Foo" else "draft"})
    send_calls = []
    monkeypatch.setattr(outreach, "send_batch", lambda *a, **kw: send_calls.append(1))

    outreach.cmd_auto_send_all(argparse.Namespace())

    assert send_calls == []
    assert "nothing to do" in capsys.readouterr().out


def test_auto_send_all_sends_every_stage_for_every_running_campaign(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    campaign_cfg = {"status": "active", "sending": {"daily_limit": 50},
                     "stages": [{"name": "intro"}, {"name": "followup1"}]}
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: campaign_cfg)
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    send_calls = []

    def fake_send_batch(cfg, sheets, accounts, stage_name, batch_size):
        send_calls.append((stage_name, batch_size))
        return [{"status": "sent"}]

    monkeypatch.setattr(outreach, "send_batch", fake_send_batch)

    outreach.cmd_auto_send_all(argparse.Namespace())

    assert send_calls == [("intro", 50), ("followup1", 50)]


def test_auto_send_all_treats_paused_and_outside_window_as_routine_not_errors(monkeypatch, capsys):
    """Most stages on most runs won't be due — that's the normal case,
    not a failure. Only a genuine exception should exit non-zero."""
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    campaign_cfg = {"status": "active", "sending": {"daily_limit": 50},
                     "stages": [{"name": "intro"}, {"name": "followup1"}]}
    ...
    def fake_send_batch(cfg, sheets, accounts, stage_name, batch_size):
        if stage_name == "intro":
            raise outreach.OutsideSendingWindowError("outside window")
        raise outreach.CampaignPausedError("paused")

    monkeypatch.setattr(outreach, "send_batch", fake_send_batch)

    outreach.cmd_auto_send_all(argparse.Namespace())  # must NOT raise SystemExit

    out = capsys.readouterr().out
    assert "skipped" in out


def test_auto_send_all_one_stage_failure_does_not_block_other_stages_or_campaigns(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo", "Bar"])
    campaign_cfg = {"status": "active", "sending": {"daily_limit": 50},
                     "stages": [{"name": "intro"}]}
    ...
    out = capsys.readouterr().out
    assert out.count("FAILED") == 2  # both campaigns' intro stage attempted despite the failure


def test_auto_send_all_broken_campaign_config_does_not_block_others(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Broken", "Fine"])
    good_cfg = {"status": "active", "sending": {"daily_limit": 50}, "stages": [{"name": "intro"}]}
    ...
    monkeypatch.setattr(outreach, "get_campaign", fake_get_campaign)
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "send_batch", lambda *a, **kw: [{"status": "sent"}])

    with pytest.raises(SystemExit):
        outreach.cmd_auto_send_all(argparse.Namespace())

    out = capsys.readouterr().out
    assert "Broken" in out and "couldn't load config" in out
    assert "Fine / intro: sent 1" in out
...
class FakeSheets:
    def __init__(self, leads):
        ...
        self.update_response_match_calls = []
        self.get_all_leads_call_count = 0

    def get_all_leads(self):
        self.get_all_leads_call_count += 1
    ...
    def get_logged_message_info(self):
        # Row numbers assigned in append order, matching the real
        # SheetsConnector's "row 1 is header, data starts at row 2" rule
        # — same convention as get_all_responses above.
        info = {}
        for i, r in enumerate(self.responses, start=2):
            message_id = r.get("MessageID", "")
            if message_id:
                info[message_id] = {"row": i, "match_method": r.get("MatchMethod", "")}
        return info

    def update_response_match(self, row_number, match_method, action_taken):
        self.update_response_match_calls.append((row_number, match_method, action_taken))
        index = row_number - 2  # row 2 is the first data row, matching append order
        if 0 <= index < len(self.responses):
            self.responses[index]["MatchMethod"] = match_method
            self.responses[index]["ActionTaken"] = action_taken
...
# ---------- _is_reply_subject_match / the Subject match-method upgrade ----------

def test_is_reply_subject_match_single_re_prefix():
    assert outreach._is_reply_subject_match("Re: DudeRobe – Meta Ad Usage Collaboration",
                                              "DudeRobe – Meta Ad Usage Collaboration") is True


def test_is_reply_subject_match_case_insensitive():
    assert outreach._is_reply_subject_match("re: Hello There", "HELLO THERE") is True


def test_is_reply_subject_match_double_re_prefix():
    """The real-world "Re: Re: <subject>" double-accumulation some mail
    clients produce, seen in an actual reported case."""
    assert outreach._is_reply_subject_match("Re: Re: DudeRobe Collab", "DudeRobe Collab") is True


def test_is_reply_subject_match_no_re_prefix_is_not_a_match():
    """A subject with no "Re:" at all isn't a reply to this thread as
    far as this signal is concerned — an identical subject with no
    reply marker is more likely a coincidence or a forward."""
    assert outreach._is_reply_subject_match("DudeRobe Collab", "DudeRobe Collab") is False


def test_is_reply_subject_match_different_subject_is_not_a_match():
    assert outreach._is_reply_subject_match("Re: Something Else Entirely", "DudeRobe Collab") is False


def test_is_reply_subject_match_blank_original_subject_never_matches():
    """An older lead with no ThreadSubject ever tracked must correctly
    fall back to the existing sender-only, never-stops behavior —
    never accidentally treated as a match."""
    assert outreach._is_reply_subject_match("Re: Anything", "") is False


def test_is_reply_subject_match_whitespace_only_original_never_matches():
    assert outreach._is_reply_subject_match("Re: Anything", "   ") is False


def test_is_reply_subject_match_survives_en_dash_becoming_hyphen():
    """The actual reported production case: the real subject contains an
    en-dash, but a subject makes a long round trip — outbound template,
    Sheet cell, SMTP, the recipient's own mail client re-encoding it,
    then MIME-decoded IMAP on the way back — and routinely returns with
    the dash or whitespace altered. Every such variant is the same
    subject to a human and silently not-a-match to a raw ==, which
    meant a genuine reply could fail subject-matching for reasons
    having nothing to do with whether it was genuinely a reply."""
    original = "DudeRobe – Meta Ad Usage Collaboration"  # en-dash
    assert outreach._is_reply_subject_match("Re: DudeRobe - Meta Ad Usage Collaboration", original) is True


def test_is_reply_subject_match_survives_non_breaking_space():
    original = "DudeRobe – Meta Ad Usage Collaboration"
    assert outreach._is_reply_subject_match("Re: DudeRobe\u00a0– Meta Ad Usage Collaboration", original) is True


def test_is_reply_subject_match_survives_collapsed_whitespace_and_tabs():
    original = "DudeRobe – Meta Ad Usage Collaboration"
    assert outreach._is_reply_subject_match("Re:  DudeRobe  –  Meta Ad\tUsage Collaboration", original) is True


def test_is_reply_subject_match_survives_em_dash_variant():
    original = "DudeRobe – Meta Ad Usage Collaboration"
    assert outreach._is_reply_subject_match("Re: DudeRobe — Meta Ad Usage Collaboration", original) is True


def test_is_reply_subject_match_normalization_does_not_make_everything_match():
    """Normalization must never become so loose that genuinely
    different subjects start matching — the whole value of this signal
    depends on it staying specific."""
    original = "DudeRobe – Meta Ad Usage Collaboration"
    assert outreach._is_reply_subject_match("Re: Something else entirely", original) is False
    assert outreach._is_reply_subject_match("Re: DudeRobe – Meta Ad Usage", original) is False
    assert outreach._is_reply_subject_match("Re: DudeRobe Meta Ad Usage Collaboration extra", original) is False


def test_check_replies_subject_match_stops_sequence_when_headers_stripped(monkeypatch):
    """The actual reported production case: a genuine reply from the
    exact lead's own email address, with 'Re: <exact original
    subject>', but with NO threading headers at all — a real,
    confirmed mail-client quirk (iOS Mail stripping In-Reply-To /
    References on short replies), not a hypothetical. This must now be
    trusted enough to stop the sequence, where a plain sender-only
    match never could."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)

    def fake_imap_fetch_recent(address, app_password, since_dt, imap_host=None, imap_port=None, imap_username=None):
        return [{
            "message_id": "<reply-ios@mail.gmail.com>", "in_reply_to": "", "references": "",
            "subject": "Re: DudeRobe – Meta Ad Usage Collaboration", "from": "stewartlyn94@gmail.com",
            "headers": {}, "body": "Interested.", "snippet": "Interested.", "date": None,
        }]
    ...
    assert len(actions) == 1
    assert actions[0]["match_method"] == "Subject"
    assert actions[0]["classification"] == outreach.CLASSIFICATION_GENUINE
...
def test_check_replies_subject_mismatch_stays_unverified_never_stops(monkeypatch):
    """Same sender email, but a subject that does NOT match this lead's
    own original thread — must stay exactly as unverified/non-stopping
    as before this feature existed. The upgrade path is narrow by
    design, not a general loosening of sender-only matching."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)
    ...
    def fake_imap_fetch_recent(address, app_password, since_dt, imap_host=None, imap_port=None, imap_username=None):
        return [{
            "message_id": "<unrelated@mail.gmail.com>", "in_reply_to": "", "references": "",
            "subject": "Completely unrelated subject", "from": "stewartlyn94@gmail.com",
            "headers": {}, "body": "Interested.", "snippet": "Interested.", "date": None,
        }]
...
def test_check_replies_subject_match_auto_reply_still_never_stops(monkeypatch):
    """A subject match alone is never sufficient — the classification
    step still independently governs whether a sequence actually
    stops. An auto-reply with a matching subject must not stop the
    sequence, exactly like a Header match wouldn't either."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)
    ...
    def fake_imap_fetch_recent(address, app_password, since_dt, imap_host=None, imap_port=None, imap_username=None):
        return [{
            "message_id": "<auto@mail.gmail.com>", "in_reply_to": "", "references": "",
            "subject": "Re: DudeRobe – Meta Ad Usage Collaboration", "from": "stewartlyn94@gmail.com",
            "headers": {"auto-submitted": "auto-replied"}, "body": "I'm away", "snippet": "I'm away",
            "date": None,
        }]
    ...
    assert actions[0]["match_method"] == "Subject"
    assert actions[0]["classification"] == outreach.CLASSIFICATION_AUTOREPLY
    assert actions[0]["action"] != outreach.ACTION_STOPPED
    updated_lead = fake_sheets._leads[0]
    assert updated_lead["ReplyStatus"] != "Replied"


def test_check_replies_header_match_still_takes_priority_over_subject(monkeypatch):
    """When a real Header match exists, it must still be used directly
    — the Subject-matching logic should never even need to run, since
    match_method is already resolved to Header first."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com",
                        MessageID="<original@mail.gmail.com>",
                        ThreadSubject="Completely different subject than the reply uses")]
    fake_sheets = FakeSheets(leads)

    def fake_imap_fetch_recent(address, app_password, since_dt, imap_host=None, imap_port=None, imap_username=None):
        return [{
            "message_id": "<reply@mail.gmail.com>", "in_reply_to": "<original@mail.gmail.com>", "references": "",
            "subject": "Re: DudeRobe – Meta Ad Usage Collaboration", "from": "stewartlyn94@gmail.com",
            "headers": {}, "body": "Interested.", "snippet": "Interested.", "date": None,
        }]
...
# ---------- re-evaluating a previously-unverified message on a later run ----------

def test_check_replies_reevaluates_and_upgrades_a_previously_unverified_message(monkeypatch):
    """The actual reported production issue, reproduced end-to-end: a
    message logged as Unverified/Email BEFORE the Subject-match logic
    existed was permanently unable to ever be reconsidered under the
    old get_logged_message_ids (a bare set with no match-method
    information) — every subsequent run correctly-by-the-old-rules
    skipped it as 'already seen', regardless of how many times
    check_replies ran or that the matching logic had since improved.
    This is the fix: a message previously logged as anything OTHER
    than Header/Subject is eligible for re-evaluation, and an upgrade
    updates the EXISTING response row and the lead's own fields."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)
    # Simulates a response already logged from an EARLIER run, before
    # Subject-match existed — MatchMethod is "Email", not "Subject".
    fake_sheets.responses.append({
        "ResponseID": "<reply-ios@mail.gmail.com>", "LeadID": "L1", "MessageID": "<reply-ios@mail.gmail.com>",
        "MatchMethod": "Email", "ActionTaken": outreach.ACTION_LOGGED_UNVERIFIED,
        "Classification": outreach.CLASSIFICATION_GENUINE, "Subject": "Re: DudeRobe – Meta Ad Usage Collaboration",
    })
    ...
    # Updated the EXISTING response row in place — no duplicate appended.
    assert len(fake_sheets.responses) == 1
    assert fake_sheets.responses[0]["MatchMethod"] == "Subject"
    assert fake_sheets.responses[0]["ActionTaken"] == outreach.ACTION_STOPPED
    assert len(fake_sheets.update_response_match_calls) == 1


def test_check_replies_previously_logged_header_or_subject_never_reevaluated(monkeypatch):
    """The other half of the guarantee: a message already logged as a
    DEFINITIVE match (Header or Subject) must never be touched again —
    re-running the same matching logic on it is pure waste, and the
    dedup skip for these must still work exactly as it always has."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)
    fake_sheets.responses.append({
        "ResponseID": "<already-resolved@mail.gmail.com>", "LeadID": "L1",
        "MessageID": "<already-resolved@mail.gmail.com>",
        "MatchMethod": "Subject", "ActionTaken": outreach.ACTION_STOPPED,
        "Classification": outreach.CLASSIFICATION_GENUINE,
    })

    def fake_imap_fetch_recent(address, app_password, since_dt, imap_host=None, imap_port=None, imap_username=None):
        return [{
            "message_id": "<already-resolved@mail.gmail.com>", "in_reply_to": "", "references": "",
            "subject": "Re: DudeRobe – Meta Ad Usage Collaboration", "from": "stewartlyn94@gmail.com",
            "headers": {}, "body": "Interested.", "snippet": "Interested.", "date": None,
        }]
    ...
    assert actions == []
    assert fake_sheets.update_response_match_calls == []
    assert len(fake_sheets.responses) == 1  # unchanged


def test_check_replies_reevaluation_that_still_doesnt_upgrade_makes_no_changes(monkeypatch):
    """A message re-evaluated but STILL only sender-matchable (subject
    genuinely doesn't confirm it either) must produce no new action,
    no duplicate row, and no lead-field changes — nothing actually
    changed, so nothing should be touched."""
    leads = [make_lead(_row=2, LeadID="L1", Email="stewartlyn94@gmail.com", MessageID="",
                        ThreadSubject="DudeRobe – Meta Ad Usage Collaboration")]
    fake_sheets = FakeSheets(leads)
    fake_sheets.responses.append({
        "ResponseID": "<unrelated@mail.gmail.com>", "LeadID": "L1", "MessageID": "<unrelated@mail.gmail.com>",
        "MatchMethod": "Email", "ActionTaken": outreach.ACTION_LOGGED_UNVERIFIED,
        "Classification": outreach.CLASSIFICATION_GENUINE,
    })
...
def test_get_logged_message_info_skips_blank_message_ids():
    fake_sheets = FakeSheets([])
    fake_sheets.responses.append({"ResponseID": "noid-123", "MessageID": "", "MatchMethod": "Email"})
    fake_sheets.responses.append({"ResponseID": "<real@id>", "MessageID": "<real@id>", "MatchMethod": "Header"})
    info = fake_sheets.get_logged_message_info()
    assert list(info.keys()) == ["<real@id>"]
    assert info["<real@id>"] == {"row": 3, "match_method": "Header"}
...
def test_call_with_gspread_retries_retries_on_429_rate_limit(monkeypatch):
    """The actual reported production issue: a 429 quota error must be
    retried with a MUCH longer backoff than a generic 5xx, since
    Google's Sheets API quota resets on a per-MINUTE window — the old
    short schedule (2s, 4s, 8s — ~14s total) could easily exhaust
    before the very same 60-second window that caused the 429 had even
    finished."""
    sleep_calls = []
    monkeypatch.setattr(outreach.time, "sleep", lambda s: sleep_calls.append(s))
    attempts = {"count": 0}
    ...
    assert sleep_calls == [15]  # the new, much longer quota-specific first delay


def test_call_with_gspread_retries_429_uses_the_full_longer_schedule(monkeypatch):
    sleep_calls = []
    monkeypatch.setattr(outreach.time, "sleep", lambda s: sleep_calls.append(s))
    attempts = {"count": 0}
    ...
    result = outreach._call_with_gspread_retries(flaky)
    assert result == "finally worked"
    assert sleep_calls == [15, 30, 60]  # the full quota-specific schedule, in order


def test_call_with_gspread_retries_429_raises_after_its_own_schedule_exhausted(monkeypatch):
    ...
    with pytest.raises(outreach.gspread.exceptions.APIError):
        outreach._call_with_gspread_retries(always_429)
    # 1 initial attempt + 3 quota retries = 4 total attempts
    assert attempts["count"] == len(outreach.GSPREAD_QUOTA_RETRY_DELAYS_SECONDS) + 1


def test_call_with_gspread_retries_429_schedule_independent_of_generic_max_retries(monkeypatch):
    """The quota-specific schedule must use its own retry budget, not
    share/consume the generic GSPREAD_MAX_RETRIES counter — a 429
    retried 3 times (its own full schedule) followed by success must
    not be treated as having exhausted the generic budget."""
    monkeypatch.setattr(outreach.time, "sleep", lambda s: None)
    attempts = {"count": 0}

    def flaky():
        attempts["count"] += 1
        if attempts["count"] <= outreach.GSPREAD_MAX_RETRIES:  # more than a naive shared-counter design would allow
            raise _make_gspread_api_error(429)
        return "ok"
...
def test_check_replies_reports_account_errors_when_asked(monkeypatch):
    """The actual gap this fixes: a caller needs a way to tell 'zero new
    messages because there genuinely were none' apart from 'zero new
    messages because an account was silently never checked at all' —
    both look identical in the returned actions list alone."""
    leads = [make_lead(_row=2, LeadID="L1", Email="john@abc.com", MessageID="")]
    fake_sheets = FakeSheets(leads)
    ...
    account_errors = []
    actions = outreach.check_replies(fake_sheets, {"sales1": ACCOUNTS["sales1"]}, lookback_hours=24,
                                      campaign_name="test_campaign", account_errors=account_errors)

    assert actions == []
    assert len(account_errors) == 1
    assert "sales1" in account_errors[0]
    assert "simulated IMAP outage" in account_errors[0]


def test_check_replies_default_call_without_account_errors_is_unaffected(monkeypatch):
    """Backward compatible — every existing caller that doesn't pass
    account_errors at all must behave exactly as before (no crash from
    account_errors being None)."""
    leads = [make_lead(_row=2, LeadID="L1", Email="john@abc.com", MessageID="")]
    fake_sheets = FakeSheets(leads)
    ...
    actions = outreach.check_replies(fake_sheets, {"sales1": ACCOUNTS["sales1"]}, lookback_hours=24,
                                      campaign_name="test_campaign")  # no account_errors passed
    assert actions == []


def test_check_replies_no_account_errors_when_all_accounts_succeed(monkeypatch):
    leads = [make_lead(_row=2, LeadID="L1", Email="john@abc.com", MessageID="")]
    fake_sheets = FakeSheets(leads)
    monkeypatch.setattr(outreach, "imap_fetch_recent", lambda *a, **kw: [])
...
def test_cmd_check_replies_surfaces_account_errors_in_stdout(monkeypatch, capsys):
    """The actual fix: this must appear in STDOUT (what the workflow's
    job summary captures via `tee`), not just stderr — otherwise a
    silent account failure looks exactly like 'genuinely no new
    replies' in the one place a person actually looks."""
    campaign_cfg = {"reply_monitor": {}}
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: campaign_cfg)
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {"sales1": ACCOUNTS["sales1"]})

    def fake_check_replies(sheets, accounts, lookback_hours, campaign_name="", anthropic_api_key=None,
                            account_errors=None):
        if account_errors is not None:
            account_errors.append("sales1: IMAPConnectionError — simulated outage")
        return []

    monkeypatch.setattr(outreach, "check_replies", fake_check_replies)

    args = argparse.Namespace(campaign="Foo")
    outreach.cmd_check_replies(args)
...
# ---------- cmd_check_replies_all ----------

def test_check_replies_all_runs_for_every_campaign_regardless_of_status(monkeypatch, capsys):
    """The actual reported production bug: the scheduled trigger only
    ever checked one hardcoded campaign name, silently missing every
    other campaign — including whatever the CURRENT actual campaign
    is, if it was created after that name was hardcoded. Also
    confirms 'regardless of status' — a paused or completed campaign
    must still be checked, since it can still receive a genuine reply
    needing a lead's sequence correctly stopped."""
    monkeypatch.setattr(outreach, "discover_campaign_names",
                         lambda: ["Kelson_Creators_Licensing", "Kelson_Creators_Licensing_03SEP"])
    monkeypatch.setattr(outreach, "get_campaign",
                         lambda name, **kw: {"status": "paused" if "03SEP" not in name else "active",
                                              "reply_monitor": {}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {"sales1": ACCOUNTS["sales1"]})
    ...
    def fake_check_replies(sheets, accounts, lookback_hours, campaign_name="", anthropic_api_key=None,
                            account_errors=None):
        checked_campaigns.append(campaign_name)
        return []

    monkeypatch.setattr(outreach, "check_replies", fake_check_replies)

    outreach.cmd_check_replies_all(argparse.Namespace())

    assert checked_campaigns == ["Kelson_Creators_Licensing", "Kelson_Creators_Licensing_03SEP"]
    out = capsys.readouterr().out
    assert "Kelson_Creators_Licensing_03SEP" in out


def test_check_replies_all_one_campaign_failure_does_not_block_others(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo", "Bar"])

    def fake_get_campaign(name, **kw):
        if name == "Foo":
            raise RuntimeError("Foo's config is broken")
        return {"reply_monitor": {}}

    monkeypatch.setattr(outreach, "get_campaign", fake_get_campaign)
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})
    monkeypatch.setattr(outreach, "check_replies", lambda *a, **kw: [])

    with pytest.raises(SystemExit):
        outreach.cmd_check_replies_all(argparse.Namespace())

    out = capsys.readouterr().out
    assert "Foo" in out and "failed entirely" in out
    assert "Bar: no new inbound messages matched." in out


def test_check_replies_all_reports_processed_messages_per_campaign(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {"reply_monitor": {}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})

    def fake_check_replies(sheets, accounts, lookback_hours, campaign_name="", anthropic_api_key=None,
                            account_errors=None):
        return [{"email": "rocky@abc.com", "classification": "Genuine Reply",
                 "match_method": "Header", "account": "sales1", "action": "Stopped Sequence"}]
    ...
    out = capsys.readouterr().out
    assert "Foo: processed 1 inbound message(s):" in out
    assert "rocky@abc.com" in out


def test_check_replies_all_no_campaigns_found(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: [])
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {})

    outreach.cmd_check_replies_all(argparse.Namespace())  # must NOT raise SystemExit

    out = capsys.readouterr().out
    assert "No campaigns found" in out


def test_check_replies_all_surfaces_account_errors_per_campaign(monkeypatch, capsys):
    monkeypatch.setattr(outreach, "discover_campaign_names", lambda: ["Foo"])
    monkeypatch.setattr(outreach, "get_campaign", lambda name, **kw: {"reply_monitor": {}})
    monkeypatch.setattr(outreach, "_connect_sheets", lambda cfg: FakeSheets([]))
    monkeypatch.setattr(outreach, "load_email_accounts", lambda: {"sales1": ACCOUNTS["sales1"]})
    ...
    out = capsys.readouterr().out
    assert "Foo" in out and "sales1" in out and "simulated outage" in out
...
# ---------- account-name case sensitivity (found by full-codebase audit) ----------

CASED_ACCOUNTS = {
    "Sales2": {"address": "a@x.com", "app_password": "p"},
    "Sales3": {"address": "b@x.com", "app_password": "p"},
    "Sales4": {"address": "c@x.com", "app_password": "p"},
}


FREEFORM_ACCOUNTS = {
    "Sales 2": {"address": "a@x.com", "app_password": "p"},
    "client_outreach-3": {"address": "b@x.com", "app_password": "p"},
    "Harish.Main": {"address": "c@x.com", "app_password": "p"},
}


def test_normalize_account_name_ignores_case_space_and_separators():
    n = outreach.normalize_account_name
    assert n("Sales 2") == n("sales-2") == n("SALES_2") == n("sales.2") == n("  sales2  ")
    assert n("") == ""


def test_normalize_account_name_keeps_genuinely_different_names_distinct():
    """Deliberately conservative — digits and words are never dropped."""
    n = outreach.normalize_account_name
    assert n("sales2") != n("sales3")
    assert n("sales") != n("sales2")


def test_canonical_account_name_accepts_any_user_naming_style():
    """The user must be able to name slots however they like and have
    every reference to that name resolve, regardless of the formatting
    used in each of the three places names are typed by hand."""
    for variant in ("sales2", "SALES 2", "sales-2", "Sales_2", "  sales 2  "):
        assert outreach.canonical_account_name(variant, FREEFORM_ACCOUNTS) == "Sales 2"
    for variant in ("CLIENT OUTREACH 3", "client-outreach_3", "clientoutreach3"):
        assert outreach.canonical_account_name(variant, FREEFORM_ACCOUNTS) == "client_outreach-3"
    for variant in ("harishmain", "Harish Main", "HARISH-MAIN"):
        assert outreach.canonical_account_name(variant, FREEFORM_ACCOUNTS) == "Harish.Main"


def test_canonical_account_name_exact_match_always_wins():
    accounts = {"Sales 2": {"address": "a"}}
    assert outreach.canonical_account_name("Sales 2", accounts) == "Sales 2"


def test_canonical_account_name_raises_on_genuinely_ambiguous_config():
    """Two DISTINCT slots differing only by separator must not be
    silently collapsed — sending from the wrong address is worse than
    a loud, fixable configuration error."""
    ambiguous = {"Sales 2": {"address": "a"}, "sales-2": {"address": "b"}}
    with pytest.raises(outreach.AmbiguousAccountNameError):
        outreach.canonical_account_name("sales2", ambiguous)


def test_rotation_accounts_accepts_freeform_naming_styles():
    cfg = {"sending": {"rotation_accounts": ["sales 2", "CLIENT-OUTREACH-3"]}}
    assert outreach.get_rotation_accounts(cfg, FREEFORM_ACCOUNTS) == ["Sales 2", "client_outreach-3"]


def test_pinned_sender_resolves_across_naming_styles():
    cfg = {"sending": {"sender_rotation": True, "rotation_accounts": ["Sales 2", "Harish.Main"]}}
    lead = make_lead(Email="r@x.com", SenderAccount="SALES_2")
    sent_today = {"Sales 2": 50, "Harish.Main": 0}  # pin must win over least-used rotation
    assert outreach._resolve_account_for_round(
        lead, cfg, FREEFORM_ACCOUNTS, {}, sent_today, set()) == ("account", "Sales 2")


def test_canonical_account_name_matches_case_insensitively():
    assert outreach.canonical_account_name("sales2", CASED_ACCOUNTS) == "Sales2"
    assert outreach.canonical_account_name("SALES3", CASED_ACCOUNTS) == "Sales3"
    assert outreach.canonical_account_name("Sales4", CASED_ACCOUNTS) == "Sales4"
    assert outreach.canonical_account_name("nope", CASED_ACCOUNTS) is None
    assert outreach.canonical_account_name("", CASED_ACCOUNTS) is None


def test_rotation_accounts_resolve_case_insensitively():
    """Real config has slots named "Sales2"/"Sales3"; a campaign config
    written as "sales2"/"sales3" previously produced an EMPTY rotation
    pool — every send in the campaign failing over capitalization."""
    cfg = {"sending": {"rotation_accounts": ["sales2", "sales3"]}}
    assert outreach.get_rotation_accounts(cfg, CASED_ACCOUNTS) == ["Sales2", "Sales3"]


def test_rotation_accounts_dedupes_case_variants():
    cfg = {"sending": {"rotation_accounts": ["sales2", "Sales2", "SALES3"]}}
    assert outreach.get_rotation_accounts(cfg, CASED_ACCOUNTS) == ["Sales2", "Sales3"]


def test_pinned_sender_case_variant_is_not_treated_as_removed():
    """The serious one: a lead pinned to "sales2" against a secret keyed
    "Sales2" was SILENTLY reassigned to a different sender mid-thread,
    because the stale-pin check saw the case variant as no longer in
    the rotation. Silent wrong-sender is worse than the loud error it
    replaced."""
    cfg = {"sending": {"sender_rotation": True, "rotation_accounts": ["Sales2", "Sales3"]}}
    lead = make_lead(Email="r@x.com", SenderAccount="sales2")
    # Sales2 is loaded with sends today, so if the pin were wrongly
    # discarded, rotation would deterministically pick the LEAST-used
    # account (Sales3) instead — making this assertion fail for the
    # right reason rather than passing by coincidence.
    sent_today = {"Sales2": 50, "Sales3": 0}
    assert outreach._resolve_account_for_round(
        lead, cfg, CASED_ACCOUNTS, {}, sent_today, set()) == ("account", "Sales2")


def test_genuinely_unknown_pinned_sender_still_errors_loudly():
    cfg = {"sending": {"sender_rotation": True, "rotation_accounts": ["Sales2"]}}
    lead = make_lead(Email="r@x.com", SenderAccount="NoSuchAccount")
    kind, value = outreach._resolve_account_for_round(lead, cfg, CASED_ACCOUNTS, {}, {}, set())
    assert kind == "error"
    assert isinstance(value, outreach.MissingSenderAccountError)


def test_genuinely_removed_sender_is_still_re_rotated():
    """The stale-pin fix must still work for a REAL removal — Sales4
    exists in the secret but is not in this campaign's rotation."""
    cfg = {"sending": {"sender_rotation": True, "rotation_accounts": ["Sales2", "Sales3"]}}
    lead = make_lead(Email="r@x.com", SenderAccount="Sales4")
    kind, value = outreach._resolve_account_for_round(lead, cfg, CASED_ACCOUNTS, {}, {}, set())
    assert kind == "account"
    assert value in ("Sales2", "Sales3")
...
# ---------- stale SenderAccount pin after a campaign's senders are changed ----------

def test_resolve_account_stale_pin_reassigned_when_removed_from_rotation():
    """The actual reported production bug: a campaign launched with one
    sender, then reconfigured (that sender REMOVED, three new ones
    added), kept sending every follow-up from the removed account
    forever — because the lead's SenderAccount was pinned on first send
    and validated only against the global EMAIL_ACCOUNTS_JSON (where the
    old account still exists), never against the campaign's own current
    rotation list. Removing an account from a campaign silently did
    nothing for any lead already contacted."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales1")  # pinned by an earlier send
    campaign_cfg = {"sending": {"sender_rotation": True,
                                 "rotation_accounts": ["sales2", "sales3"]}}  # sales1 removed
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS,
        confirmed_batch_counts={}, sent_today_by_account={}, used_accounts_this_round=set())

    assert kind == "account"
    assert value in ("sales2", "sales3")
    assert value != "sales1"  # never the removed account


def test_resolve_account_pin_still_honored_when_still_in_rotation():
    """The original "locked in" guarantee must still hold — a pinned
    account that IS still part of the campaign's current senders keeps
    winning, so a thread's follow-ups still come from the same address
    as its intro."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales1")
    campaign_cfg = {"sending": {"sender_rotation": True,
    ...
    assert (kind, value) == ("account", "sales1")


def test_resolve_account_pin_untouched_when_rotation_disabled():
    """With rotation OFF there's no campaign-level sender list to check
    a pin against, so the pin must be left exactly as-is — this fix
    must never change behavior for a single-sender campaign."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales1")
    campaign_cfg = {"sending": {"sender_rotation": False}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS,
        confirmed_batch_counts={}, sent_today_by_account={}, used_accounts_this_round=set())
...
def test_resolve_account_stale_pin_not_dropped_when_rotation_list_empty():
    """An empty/unresolvable rotation list must never cause a valid pin
    to be discarded — that would turn a config problem into silently
    sending from an unexpected account."""
    lead = make_lead(Email="rocky@abc.com", SenderAccount="sales1")
    campaign_cfg = {"sending": {"sender_rotation": True, "rotation_accounts": ["nonexistent_account"]}}
    kind, value = outreach._resolve_account_for_round(
        lead, campaign_cfg, ROTATION_ACCOUNTS,
        confirmed_batch_counts={}, sent_today_by_account={}, used_accounts_this_round=set())
...
# =============================================================================
# parse_stages_and_variants_from_filenames — the pure logic underneath
# discover_stages_and_variants, decoupled from the local filesystem, so a
# caller with a filename list from GitHub's own API (bypassing whatever
# Streamlit's local checkout currently believes) can validate against it.
# =============================================================================

def test_parse_stages_and_variants_matches_local_disk_behavior_for_valid_campaign():
    filenames = ["intro_A.txt", "intro_B.txt", "followup1_A.txt", "followup1_B.txt"]
...
def test_parse_stages_and_variants_detects_the_exact_reported_bug():
    """Reproduces the real production case: intro/followup1/followup2
    all have A-D, followup3 only has C and D (A and B missing) — must
    raise naming followup3 specifically, with A and B as the missing
    ones, exactly matching what discover_stages_and_variants would say
    against a fully-synced local disk with the same files."""
    filenames = (
    ...
    with pytest.raises(outreach.ConfigError) as exc_info:
        outreach.parse_stages_and_variants_from_filenames(filenames, {})
    message = str(exc_info.value)
    assert "followup3" in message
    assert "['A', 'B']" in message


def test_parse_stages_and_variants_stops_at_first_gap():
    filenames = ["intro_A.txt", "followup2_A.txt"]  # followup1 missing entirely
    stages, variants = outreach.parse_stages_and_variants_from_filenames(filenames, {})
    assert [s["template_prefix"] for s in stages] == ["intro"]


def test_parse_stages_and_variants_empty_list_raises():
    with pytest.raises(outreach.ConfigError, match="No template files found"):
        outreach.parse_stages_and_variants_from_filenames([], {})


def test_parse_stages_and_variants_includes_source_description_in_error():
    with pytest.raises(outreach.ConfigError, match="No template files found in templates/Foo"):
        outreach.parse_stages_and_variants_from_filenames([], {}, source_description=" in templates/Foo")


def test_parse_stages_and_variants_ignores_unrelated_filenames():
    filenames = ["intro_A.txt", "intro_B.txt", "README.md", "notes.txt"]
    stages, variants = outreach.parse_stages_and_variants_from_filenames(filenames, {})
    assert variants == ["A", "B"]


def test_discover_stages_and_variants_delegates_to_the_pure_function(tmp_path):
    """discover_stages_and_variants is now a thin local-disk wrapper —
    confirms it produces IDENTICAL results to calling the pure function
    directly with the same filenames, so the two can never quietly
    drift apart."""
    d = tmp_path / "templates"
    d.mkdir()
    for name in ["intro_A.txt", "intro_B.txt", "followup1_A.txt", "followup1_B.txt"]:
        (d / name).write_text("Subject: Hi\n\nBody.")

    from_disk = outreach.discover_stages_and_variants(str(d), {})
    from_pure = outreach.parse_stages_and_variants_from_filenames(
        ["intro_A.txt", "intro_B.txt", "followup1_A.txt", "followup1_B.txt"], {})
    assert from_disk == from_pure


# =============================================================================
# parse_template_content — the pure logic underneath load_template,
# decoupled from the local filesystem, so a caller with content fetched
# fresh from GitHub's API (bypassing whatever Streamlit's local checkout
# currently believes) can validate and parse it directly.
# =============================================================================

def test_parse_template_content_matches_local_disk_behavior():
    content = "Subject: Hello there\n\nHi {{FirstName}},\n\nBody text."
    result = outreach.parse_template_content(content)
    assert result["subject"] == "Hello there"
    assert result["body"] == "Hi {{FirstName}},\n\nBody text."


def test_parse_template_content_missing_subject_line_raises():
    with pytest.raises(outreach.TemplateError, match="must start with a 'Subject: ...' line"):
        outreach.parse_template_content("Hi there, no subject line at all.")


def test_parse_template_content_includes_source_description_in_error():
    with pytest.raises(outreach.TemplateError, match="Template templates/Foo/intro_A.txt must start"):
        outreach.parse_template_content("no subject here", source_description=" templates/Foo/intro_A.txt")


def test_parse_template_content_blank_subject_is_allowed():
    """A blank Subject line is a deliberate feature — it means 'continue
    the existing thread' rather than starting a new one (see
    render_email) — must not be rejected as invalid."""
    result = outreach.parse_template_content("Subject: \n\nBody only.")
    assert result["subject"] == ""
    assert result["body"] == "Body only."


def test_load_template_delegates_to_the_pure_function(tmp_path):
    """load_template is now a thin local-disk wrapper — confirms it
    produces IDENTICAL results to calling the pure function directly
    with the same content, so the two can never quietly drift apart."""
    d = tmp_path / "templates" / "Foo"
    d.mkdir(parents=True)
    content = "Subject: Hello\n\nBody text here."
    (d / "intro_A.txt").write_text(content)

    from_disk = outreach.load_template(str(d), "intro", "A")
    from_pure = outreach.parse_template_content(content)
    assert from_disk == from_pure


def test_load_template_missing_file_raises_before_reaching_parse(tmp_path):
    d = tmp_path / "templates" / "Foo"
    d.mkdir(parents=True)
    with pytest.raises(outreach.TemplateError, match="Template file not found"):
        outreach.load_template(str(d), "intro", "A")
