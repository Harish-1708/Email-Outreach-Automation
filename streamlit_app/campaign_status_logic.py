...
def compute_campaign_is_complete(campaign_cfg: Dict, leads: List[Dict]) -> bool:
    """True only if there's at least one lead with an email AND every one
    of them is finished — an empty/not-yet-populated campaign is NOT
    "completed", it just hasn't started. Approval is informational only
    and no longer affects this."""
